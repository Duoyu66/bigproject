    ...
    <el-row>
      ...
      <el-col :span="10">
        <el-upload ref="upload" class="upload-demo" :action="action" :on-preview="handlePreview"
        	...
        	:on-success="onSuccess" :list-type="listtype">
        	<el-button v-show="!only_show" slot="trigger" size="small" type="primary" :disabled="disabled_upload">选择<i
        			class="el-icon-tickets"></i></el-button>
        	<el-tooltip class="item" effect="dark" content="预览上传文件" placement="top-start">
        		<el-button style="margin-left: 10px" size="small" type="success" v-show="service_retain.length > 0"
        			@click="preview_upload">
        			<i class="el-icon-view"></i>
        		</el-button>
        	</el-tooltip>
        	<el-tooltip class="item" effect="dark" content="下载文件" placement="top-start">
        		<el-button type="info" v-show="service_retain.length > 0" @click="downFile"><i
        				class="el-icon-download"></i>
        		</el-button>
        	</el-tooltip>
        	<el-tooltip class="item" effect="dark" content="无文件查看" placement="top-start">
        		<el-button style="margin-left: 10px" size="small" type="success"
        			v-show="only_show &&service_retain.length <= 0" :disabled="true">
        			无文件查看
        		</el-button>
        	</el-tooltip>
        	<div slot="tip" class="el-upload__tip" v-if="tip_show">
        		只能上传{{ this.type_str }}文件，且不超过{{
          this.file_size + this.size_unit
        }}
        	</div>
    ...
    //   },
      ...
      fill_data: {
		...
		watch: {
			...
			upload_type(newVal, oldVal) {
				this.action = this.static_var.gtConfig_fob.UploadFileNameUrl + '?fileName=' + this.fileName +'&fileType=' + newVal
			},
        ...
        fileName: '',
				...
				action: this.static_var.gtConfig_fob.UploadFileNameUrl + '?fileName=' + this.fileName +'&fileType=' + this.upload_type,
				max_size: -1,
				success: false,
				...
				err: '', //错误信息
				file: {
					original_name: this.original_name_json,
				},
        restaurants: [],
			}
		},
		methods: {
			clear() {
				this.value_update = false
				this.fileList = []
				this.action = this.static_var.gtConfig_fob.UploadFileNameUrl + '?fileName=' + this.fileName +'&fileType=' + this.upload_type,
				this.success = false
				this.serve_url = {}
      ...
      blurChange() {
        ...
        this.action = this.static_var.gtConfig_fob.UploadFileNameUrl + '?fileName=' + this.fileName +'&fileType=' + this.upload_type
				...
				const type =
					that.type_array.indexOf(file.raw.type) >= 0 ?
					true :
					false
				const size = file.size / 1024 / 1024 < that.GetMaxSize()
				if (!type) {
					this.$message.error(
						'上传格式只能是' + that.type_str
					)
				}
				if (!size) {
					this.$message.error(
				...
				const hw = await readSizeEx(file.raw)
				let hwFlag = true
				if (hw[0] > 0 && hw[1] > 0 && (hw[0] < that.img_height || hw[1] < that.img_width)) {
					hwFlag = false
					this.$message.error(
						'图片分辨率需大于' +
						that.img_width + '*' +
			...
			GetOriginalNameEx(absoluteUrl) {
				try {
					const prefix = ViewfileUrl(this.upload_type)
