	...
	<div class="wl-transfer transfer" :style="{ width, height }">
			...
			<div class="transfer-left">
				...
				<div class="transfer-main">
					<div>
						...
						<el-tree ref="from-tree" show-checkbox :lazy="lazy" :indent="indent" :draggable="draggable"
							:allow-drag="allowDrag" :allow-drop="allowDrop" :icon-class="iconClass" :node-key="node_key"
							:load="leftloadNode" :props="defaultProps" :data="self_from_data" :accordion="accordion"
							...
							@node-drag-start="nodeDragStartLeft" @node-drag-enter="nodeDragEnterLeft"
							@node-drag-leave="nodeDragLeaveLeft" @node-drag-over="nodeDragOverLeft"
					...
					<el-pagination v-show="pagination" @size-change="self_toSizeChange"
						@current-change="self_toCurrentChange" :current-page.sync="self_tocurrentPage1"
						:page-size="self_to_page_size" layout="prev,jumper, next" :total="from_data_total"
						prev-text="上一页" next-text="下一页" :small="true" background>
					</el-pagination>
					<!--  -->
			...
			<div class="transfer-right">
				...
				<div class="transfer-main">
					...
					<el-input v-if="filter" clearable size="small" v-model="filterTo" :placeholder="placeholder"
						class="filter-tree"></el-input>
					<el-tree slot="to" ref="to-tree" show-checkbox :indent="indent" :draggable="draggable"
						:allow-drag="allowDrag" :allow-drop="allowDrop" :icon-class="iconClass" :lazy="lazyRight"
						:data="self_to_data" :node-key="node_key" :props="defaultProps" :load="rightloadNode"
						:default-expand-all="openAll" :highlight-current="highLight" :check-strictly="checkStrictly"
						:filter-node-method="filterNodeTo" :render-content="renderContentRight"
						:check-on-click-node="checkOnClickNode" :render-after-expand="renderAfterExpand"
						:expand-on-click-node="expandOnClickNode" :default-expanded-keys="to_expanded_keys"
						@check="toTreeChecked" @node-drag-start="nodeDragStartRight"
						@node-drag-enter="nodeDragEnterRight" @node-drag-leave="nodeDragLeaveRight"
						@node-drag-over="nodeDragOverRight" @node-drag-end="nodeDragEndRight"
		...
		<template v-else-if="mode == 'addressList'">
			...
			<div class="transfer-left">
				...
				<div class="transfer-main">
					<slot name="from"></slot>
					<el-input v-if="filter" clearable size="small" class="filter-tree" :placeholder="placeholder"
						v-model="filterFrom"></el-input>
					<el-tree ref="from-tree" show-checkbox :indent="indent" :draggable="draggable"
						:allow-drag="allowDrag" :allow-drop="allowDrop" :icon-class="iconClass" :node-key="node_key"
						:props="defaultProps" :data="self_from_data" :default-expand-all="openAll"
						:highlight-current="highLight" :render-content="renderContentLeft"
						:filter-node-method="filterNodeFrom" :check-on-click-node="checkOnClickNode"
						:render-after-expand="renderAfterExpand" :expand-on-click-node="expandOnClickNode"
						:default-expanded-keys="from_expanded_keys" @check="fromTreeChecked"
						@node-drag-start="nodeDragStartLeft" @node-drag-enter="nodeDragEnterLeft"
						@node-drag-leave="nodeDragLeaveLeft" @node-drag-over="nodeDragOverLeft"
						@node-drag-end="nodeDragEndLeft" @node-drop="nodeDropLeft"></el-tree>
				</div>
			</div>
			<!-- 穿梭区 按钮框 -->
			<div class="transfer-center address-list-center">
				...
				<p class="transfer-center-item" v-if="addressOptions.num > 1">
					<el-button type="primary" @click="addressListTransfer(1)" :disabled="from_disabled"
						icon="el-icon-arrow-right" circle></el-button>
				</p>
				<p class="transfer-center-item" v-show="move_up">
					<el-button type="primary" @click="addressListTransfer(2)" :disabled="from_disabled"
						icon="el-icon-arrow-right" circle></el-button>
				</p>
			</div>
			<!-- 右侧列表区 -->
            ...
            'transfer-right-only': addressOptions.num === 1,
					...
					<div class="transfer-main" v-if="!move_up">
						<slot name="to"></slot>
						<el-input v-if="filter" :placeholder="placeholder" v-model="filterListFirst" size="small"
							class="filter-tree"></el-input>
				...
				<div v-if="addressOptions.num === 3" class="transfer-right-item"
					...
					<div class="transfer-main" v-if="move_up">
						...
						<ul class="address-list-ul">
							<li class="address-list-li" v-for="item of secret_receiver" :key="item[node_key]">
								...
								<i class="address-list-del el-icon-delete" @click="clearList(2, item[node_key])">
								</i>
							</li>
						</ul>
					</div>
...
<script>
	...
	export default {
		data() {
			return {
				self_tocurrentPage1: 1,
				self_to_page_size: 20,
		...
		methods: {
			//pageSize 改变时会触发
			self_toSizeChange(val) {
				console.log('pageSize',val)
				this.lazyFn && this.lazyFn({
					pageSize: val,
					currentPage: this.self_tocurrentPage1,
					rootPidValue:this.rootPidValue,
					filterFrom:this.filterFrom
				}, undefined, "left-pag");
			},
			//currentPage 改变时会触发
			self_toCurrentChange(val) {
				console.log('currentPage',val)
				this.lazyFn && this.lazyFn({
					pageSize: this.self_to_page_size,
					currentPage: val,
					rootPidValue:this.rootPidValue,
					filterFrom:this.filterFrom
				}, undefined, "left-pag");
