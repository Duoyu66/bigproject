<template>
	<div class="el-quarter-picker">
		<el-popover v-model="visible" :disabled="!canPopover" :tabindex="null" :placement="placement"
			transition="el-zoom-in-top" trigger="click">

			<div class="el-date-picker">
				<div class="el-picker-panel__body">
					<div class="el-date-picker__header el-date-picker__header--bordered"
						style="margin:0px; line-height:30px">
						<button type="button" @click="clickLast" aria-label="前一年"
						...
						<button type="button" @click="clickNext" aria-label="后一年"
							...
							<tbody>
								<tr v-for="line in lineCount" :key="line">
									<td v-for="index in (line * 4 <= viewList.length ? 4 : viewList.length - (line - 1) * 4)"
										...
										<div><a class="cell"
												@click="clickItem(viewList[(line - 1) * 4 + index - 1])">{{ viewList[(line - 1) * 4 + index - 1].label }}</a>
										</div>
									</td>
								</tr>
							</tbody>
			...
			<el-input slot="reference" @change="changeText" @mouseenter.native="mouseEnter"
				@mouseleave.native="mouseLeave" :placeholder="placeholder" v-model="text" :size="size"
				:readonly="!canEdit" :disabled="disabled">
				<i slot="prefix" class="el-input__icon el-icon-date"></i>
				<i slot="suffix" class="el-input__icon el-icon-circle-close" v-show="showClear" style="cursor:pointer"
					@click.stop="clear"></i>
			</el-input>
		</el-popover>
	</div>
...
<script>
	export default {
		...
		methods: {
			...
			format_fob(date, fmt = 'yyyy-MM-dd') {
				try {
			...
			changeToDate(format) {
				...
				let d = new Date(data[0], month.end, 0);
				let day = d.getDate();
				let endDate = new Date(data[0] + '-' + month.end + '-' + day);
				endDate = this.format_fob(endDate, format);
				let startDate = new Date(data[0] + '-' + month.start + '-1');
				startDate = this.format_fob(startDate, format);
				return {
					startDate: startDate,
					endDate: endDate
				}
			},
			changeToDateTime(format) {
				if (!format) {
					format = 'yyyy-MM-dd HH:mm:ss'
				}
				let range = this.changeToDate('yyyy-MM-dd');
				let endDate = new Date(range.endDate + ' 23:59:59');
				endDate = this.format_fob(endDate, format);
				let startDate = new Date(range.startDate + ' 00:00:00');
				startDate = this.format_fob(startDate, format);
				return {
					startDate: startDate,
					endDate: endDate
