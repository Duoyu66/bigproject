...
<script>
	...
	export default {
				...
				switch (this.type) {
					case 'room_type':
						if (newVal && newVal.room_no) {
							...
							//this.$emit('update:primary_org_id', Number(newVal.org_id));
						}
						break
				}
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					room_type: {
						...
						edit: 'RoomdefinitionEdit',
						delete: 'RoomdefinitionDelete',
					},
				}
				switch (name) {
			...
			handleDelete(row) {
				const vue = this
				let tips = '此操作将永久删除管理编号[' + row.room_no + '],名称[' + row.name + ']的检测室, 是否继续?'
				let sqlcode = 626
			...
			async fetchData(pageInfo, treeNode, resolve) {
				...
				switch (that.type) {
					case 'room_type':
						await that.fetchDataRoomType(pageInfo, treeNode, resolve)
			...
			async fetchDataRoomType(pageInfo, treeNode, resolve) {
				let alist = [{
						field: 'name',
						type: 2,
						level: 1,
				...
				await this.search_fob(alist, this, 625, key_json)
			},
