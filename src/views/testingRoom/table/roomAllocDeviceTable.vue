<template>
	<div>
		<el-table ref="tableSort" v-loading="listLoading" highlight-current-row :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip prop="device_name" label="设备名称"></el-table-column>
			<el-table-column show-overflow-tooltip prop="type" label="类型" width="120px"></el-table-column>
			<el-table-column show-overflow-tooltip prop="alloc_name" label="分配人" min-width="150px"></el-table-column>
			<el-table-column show-overflow-tooltip prop="alloc_time" label="分配时间" min-width="150px"></el-table-column>
...
<script>
	...
	import TableEdit from '../component/roomAllocDeviceDialog'
	export default {
		name: 'roomAllocDeviceTable',
		components: {
			TableEdit
		},
		props: {
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					room_book: {
						...
						edit: 'testRoomDeviceEdit',
						delete: 'testRoomDeviceDelete',
					},
				}
				switch (name) {
			...
			handleEdit(row) {
				...
				this.$refs['edit'].showEdit(row, undefined, option)
			},
			handleDelete(row) {
				const vue = this
				let tips = '此操作将永久删除管理编号[' + row.room_no + ']下，名称[' + row.device_name + '], 是否继续?'
				let sqlcode = 666
				this.delete_from_fob(vue, row, sqlcode, {
					//"selectRows": "selectRows", //复选框选中数据
					"fetchData": "fetchData", //加载数据
					"main_key": "alloc_id", //主键
					'confirmMsg': tips,
					'successMsg': '删除成功',
				});
			...
			async fetchDataRoomBook(pageInfo, treeNode, resolve) {
				...
				await this.search_fob(alist, this, 665, key_json)
