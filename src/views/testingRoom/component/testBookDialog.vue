<template>
	<el-dialog :title="title" :visible.sync="dialogFormVisible" top="3vh" destroy-on-close>
		<div style="max-height: calc(100vh - 220px); overflow-y: auto">
			<testBookForm ref="testBookForm" :formData="form"></testBookForm>
		</div>
...
<script>
	import testBookForm from '../form/testBookForm.vue'

	export default {
		name: 'testBookFormDialog',
		components: {
			testBookForm,
		},
		data() {
		...
		methods: {
			...
			handleUpdate() {
				let that = this
				this.$refs['testBookForm'].save(function(status) {
					if (status) {
