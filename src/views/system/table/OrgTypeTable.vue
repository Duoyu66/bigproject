  ...
  <!-- 机构类型定义 -->
	<div>
		<el-table ref="personel" v-loading="listLoading" :data="tablelist" border
			...
			<el-table-column v-if="hideColFob('type_id')" show-overflow-tooltip label="类型ID" prop="type_id"
				width="80px"></el-table-column>
			<el-table-column show-overflow-tooltip label="类型名称" prop="type_name"></el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			EditInfo(row) {
				this.$refs['OrgTypeDialog'].showEdit(row)
			},
			...
			async fetchDataMain() {
				...
				that.search_fob(select_list, that, 946, {
