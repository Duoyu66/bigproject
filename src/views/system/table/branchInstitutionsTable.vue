<template>
	<div>
		...
		<branchInstitutionsDialog ref="branchInstitutionsDialog" :is-up-data.sync="main_dialog">
	...
	export default {
		...
		methods: {
			...
			handleAdd(row, parent_info, disable) {
				this.$refs['branchInstitutionsDialog'].showEdit(row, parent_info, disable)
			},
			handleAddLevel(row) {
				let json = {
					parent_id: Object.assign({}, row).parent_id,
			...
			handleDeletemodify(row) {
				...
				that.delete_from_fob(that, row, 324, {
					...
					that.fetchData({
						org_id: row.parent_id,
						parent_id: row.parent_id,
						double_parent_id: row.parent_id,
						info: row,
						type: '重载'
					})
				})
