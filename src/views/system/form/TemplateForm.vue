			...
			<el-row>
				<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
					<el-form-item label="模板名称" :label-width="formLabelWidth" prop="template_name">
						...
						<el-select v-model="form.template_name" filterable remote placeholder="请输入关键词" v-else
				...
				<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
					<el-form-item label="页签数目" :label-width="formLabelWidth" prop="template_sheetcount">
						<el-input v-model="form.template_sheetcount" style="width: 100%;"></el-input>
					</el-form-item>
				</el-col>
				...
				<!-- <el-col :xs="24" :sm="38" :md="24" :lg="24" :xl="24">
...
<script>
	...
	import templateSheetTips from '../../testDevelopment/lucksheet/templateSheetTips.vue'
	export default {
		name: 'TableEdit',
		components: {
			tempupload,
		...
		methods: {
			...
			async templatechange(row) {
				...
				this.form.template_sheetcount = res.data[0].template_sheetcount
				setTimeout(() => {
			...
			tipsIsShow(event, isShow) {
				...
				if (isShow) {
					this.SetIframeStyle('height', '70%')
					this.button.tips.txt = this.button.tips.txtList[0]
				} else {
					this.SetIframeStyle('height', '87%')
			...
			displayold(row) {
				...
				that.form.object_id = row.param_id
      	...
      	setTimeout(() => {
			...
			save() {
				...
				this.$refs['form'].validate(async (valid) => {
					if (valid) {
            let sData = '[Q]689{' + that.form.template_name + '$`' +
            	that.form.template_sheetcount + '$`' + that.form.template_json +'}|^2|^SYS'
			...
			editsave() {
				...
				this.$refs['form'].validate(async (valid) => {
					...
					if (valid) {
						let sData = '[Q]770{' + that.form.template_name + '$`' +
            	that.form.template_json + '$`' + that.form.template_sheetcount + '$`' + that.form.template_id+
							'}|^1|^SYS'
						const res = await ExecSql(sData)
