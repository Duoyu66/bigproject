  ...
  <div class="table-container definition purchase">
    <vab-query-form>
      <vab-query-form-left-panel>
        ...
        <el-dialog title="添加校检记录" :visible.sync="dialogFormVisible">
          <el-form :model="form">
    ...
    <el-table ref="tableSort" v-loading="listLoading" :data="list" border :element-loading-text="elementLoadingText"
      ...
      <el-table-column show-overflow-tooltip label="设备类型" prop="author"></el-table-column>
      <el-table-column show-overflow-tooltip label="存放位置" prop="author"></el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(row) {
        if (row.id) {
          this.$baseConfirm('你确定要删除管理编号为()的记录吗', null, async () => {
            const {
              msg
            } = await doDelete({
              ids: row.id
