		...
		<el-row>
			...
			<el-col :span="span.full">
				<el-form-item label="" :label-width="formLabelWidth" prop="">
					<el-alert title="提示" description="加载化学品类型的前50条数据,如没有需要的请在下拉框中输入关键词进行搜索" type="info"
						:closable="false" center show-icon> </el-alert>
				</el-form-item>
			</el-col>
			<el-col :span="span.full">
				<el-form-item label="化学品名称" :label-width="formLabelWidth" prop="chemical_id">
					<el-select v-model="form.chemical_id" style="width: 100%" :disabled="readonly"
						:multiple="select.chemical_id.multiple" clearable filterable remote
						:remote-method="chemicalRemoteMethod" @change="chemicalChange">
						<el-option v-for="data in selectData.chemical_id" :key="data.item_id"
							:label=" data.model&&data.model!='/' ?data.item_name+' 规格:'+data.model :data.item_name"
							:value="data.item_id">
							<span style="float: left">{{ data.item_name }}</span>
							<span style="margin-left: 10px; color: #8492a6; font-size: 13px"
								v-show="data.model&&data.model!='/'">规格：{{ data.model }}</span>
							<span style="margin-left: 10px; color: #8492a6; font-size: 13px"
								v-show="data.unit&&data.unit!='/'">单位：{{ data.unit }}</span>
						</el-option>
					</el-select>
				</el-form-item>
			</el-col>
			<el-col :span="span.full">
				<el-form-item label="标准规范" :label-width="formLabelWidth" prop="tpl_standard">
					<paramStandCascader ref="paramStandCascader" v-model="form.tpl_standard" :param_id="form.param_id"
						:AddOrEdit="standardAddOrEdit" style="width:100%" :isMultiple="false"></paramStandCascader>
				</el-form-item>
			</el-col>

			<el-col :span="span.full">
				<el-form-item :label="chemical.unit ? '用量('+chemical.unit+')' : '用量'" :label-width="formLabelWidth"
					prop="quantity">
					<el-input v-model="form.quantity" :placeholder="isReadOnly('')" style="width:100%">
					</el-input>
				</el-form-item>
			</el-col>
	...
	export default {
		...
		methods: {
			...
			async init() {
				...
				switch (this.type) {
					...
					case 0:
						this.readonly = false
						this.form.device_id = ''
						this.select.chemical_id.multiple = false
						this.rules = this.rules_temp
						break;
						//编辑
					case 10:
						this.readonly = false
						this.form.chemical_id = ''
						this.select.chemical_id.multiple = false
						this.rules = this.rules_temp
					case 2:
						this.readonly = false
				...
				if (this.formData) {
					if (this.formData.use_id && Number(this.formData.use_id) > 0) {
			...
			async getParam() {
				...
				let list = await GetParameter(undefined, param_id)
				if (list.length > 0) {
					this.form.param = list[0].param
				}
			},
			chang_org_id() {
			...
			chemicalRemoteMethod(query) {
				this.GetChemicalId(query)
			},
			async GetGroup() {
				let that = this
				...
				await this.GetChemicalId()
			},
			chemicalChange(value) {
				let data = this.selectData.chemical_id
				let tempList = data.filter((e) => {
					return e.item_id == value
				})
				if (tempList.length == 0) {
					this.chemical.unit = ''
				} else {
					this.chemical.unit = tempList[0].unit
				}

			},
			async GetChemicalId(searchKey) {
				let chemical_id = this.form.chemical_id
				let param_id = this.form.param_id
				let chemical_id_typeof = (typeof chemical_id).toLowerCase()
				if (isNumEx(chemical_id)) {
					chemical_id = Number(chemical_id)
				}
				switch (chemical_id_typeof) {
					case 'object':
						try {
							chemical_id = chemical_id.join(',')
						} catch (e) {
							console.log(e)
						}
						break
