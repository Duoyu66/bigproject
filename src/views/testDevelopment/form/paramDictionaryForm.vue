<template>
	<span class="purchasefrom">
        ...
        <el-col :span="9">
          ...
          <el-button style type="primary" plain @click="datagroupadd" circle style="position:absolute;top:141px;left:323px;width:28px;height:28px;font-size:18px;padding:0px;" >+</el-button>
        </el-col>
        <el-col :span="9" >
        ...
        <el-col :span="6">
          <el-dropdown style="margin:0 0px 11px 35px;width: 160px;" trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(meta,index) in meta_name_options" :key="index">{{meta.meta_name}}</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown><br>
  ...
  import dataGroup from '../components/dataGroup.vue'
    ...
    components: {
      ...
      datagroupadd() {
        ...
        let sData = '[Q]476{' + this.form.template_id + '}|^SYS'
        const res = await QueryExec(sData)
        console.log(sData)
      		...
      		if (str[i] === '{') {
      			...
      			for (k = 0; k < res.total; k++) {
      				...
      				if (res.data[k].meta_name === temp) {
      					that.form.expression = that.form.expression.replace(temp, that.form.template_id +'-'+ res.data[k].meta_id)
                break
      				}
      			}
