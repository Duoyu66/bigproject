<template>
	<span class="purchasefrom">
        ...
        <el-col :span="6"  style="text-align: right;">
        	<el-form-item label="属于进场" :label-width="formLabelWidth">
        		<el-switch v-model="form.in_entry" active-value="1" inactive-value="0" class="changeWidth" style="margin-right: 15px;"></el-switch>
        	</el-form-item>
        </el-col>
				<el-col :span="9"  >
        ...
        <el-col :span="9">
        	<el-form-item label="分组来源" :label-width="formLabelWidth">
        		<el-select v-model="form.group_id" style="width: 100%;" filterable placeholder="请选择" clearable>
        			<el-option v-for="item in item_group_options" :key="item.item_name" :label="item.item_name"
        				:value="item.item_id">
        			</el-option>
        		</el-select>
        	</el-form-item>
          <el-button style type="primary" plain @click="datagroupadd" circle style="position:absolute;top:191px;left:323px;width:28px;height:28px;font-size:18px;padding:0px;" >+</el-button>
        </el-col>

         <el-col :span="9">
         	<el-form-item label="度量单位" :label-width="formLabelWidth">
         		<el-select v-model="form.metric_unit" style="width: 100%;" filterable placeholder="请选择" clearable>
        ...
        <el-col :span="9">
        	<el-form-item label="修约方式" :label-width="formLabelWidth">
        		<el-select v-model="form.rounding" style="width: 100%;" filterable placeholder="请选择" clearable>
        ...
        <el-col :span="9">
        	<el-form-item label="度量类型" :label-width="formLabelWidth">
        		<el-select v-model="form.metric" style="width: 100%;" filterable placeholder="请选择" clearable>
        ...
        <el-col :span="9" >
        	<el-form-item label="试验项目" :label-width="formLabelWidth">
        		<el-select v-model="form.belong" style="width: 100%;" filterable placeholder="请选择" clearable>
        			<el-option v-for="item in param_options" :key="item.item_id"
				...
				<el-col :span="6"  style="text-align: right;">
					<el-form-item label="属于报验" :label-width="formLabelWidth">
						<el-switch v-model="form.in_check" active-value="1" inactive-value="0" class="changeWidth" style="margin-right: 15px;"></el-switch>
					</el-form-item>
				</el-col>

        <el-col :span="9">
          <el-form-item label="台账映射" :label-width="formLabelWidth" style="position: relative; left:-199px;">
          	<el-select v-model="form.meta_desc" style="width: 100%;" filterable placeholder="请选择" clearable >
        ...
        <el-col :span="6"  style="text-align: right;">
        	<el-form-item label="需要判定" :label-width="formLabelWidth" style="position: relative; left:199px;">
        		<el-switch v-model="form.need_judge" active-value="1" inactive-value="0" class="changeWidth" style="margin-right: 15px;"></el-switch>
        	</el-form-item>
        </el-col>
        <el-col :span="18" >
        	<el-form-item label="平行插件" :label-width="formLabelWidth" style="width:710px;">
        		<el-input v-model="form.samples_proc" style="width: 100%"></el-input>
        	</el-form-item>
        </el-col>
        <el-col :span="6"  style="text-align: right;">
        	<el-form-item label="属配料单" :label-width="formLabelWidth">
        		<el-switch v-model="form.in_mixed" active-value="1" inactive-value="0" class="changeWidth" style="margin-right: 15px;"></el-switch>
        	</el-form-item>
        </el-col>
        <el-col :span="18">
        ...
        <el-col :span="6">
					<el-button style type="primary" plain style="margin:0 0px 11px 35px;width: 160px;" icon="el-icon-bottom-left">插入指标</el-button><br>
    ...
    components: {
      ...
      var validate_number = (rule, value, callback) => {
        ...
        value: [],
          ...
          samples_number: [{ validator: validate_number, trigger: 'blur' }],
				},
				item_group_options: [],
        param_options: [],
      ...
      datagroupadd() {
        ...
        this.form.meta = row.meta_id + '_' + row.meta_name
        this.form.meta_desc = row.share_name
        // let sData = '[Q]476{' + row.object_id + '}|^SYS'
        // const res1 = await QueryExec(sData)
        // console.log(sData)
        // this.formulachangex(res1)
        let sData1 = '[Q]1050{'+this.form.object_id+'}|^SYS'
        const res1 = await QueryExec(sData1)
        this.param_options=res1.data
      ...
      formulachange(res) {
        ...
        let str = ex.split('')
          ...
          temp = ''
              ...
              console.log(res.data[k].meta_name,temp)
      				if (res.data[k].meta_name === temp) {
      					that.form.expression = that.form.expression.replace(temp, that.form.template_id +'_'+ res.data[k].meta_id)
                break
      				}
      			}
      			if (k === res.total) {
        ...
        let str = ex.split('')
          ...
          temp = ''
      		if (str[i] === '{') {
      			...
      			for (let k = 0; k < res.total; k++) {
              console.log(res.data[k].meta_name,temp)
      				if (res.data[k].meta_id === temp.split('_')[1]) {
      	...
      	return true
			...
			async save(symbol) {
				...
				let meta = this.form.meta.split("_")
    ...
    //     console.log(res1)
        ...
        if(ju)
        {
          this.$refs['form'].validate(async (valid) => {
            if (valid) {
              if(that.form.item_category === '记录项' && Number(that.form.max_samples)>1)
              {
                let sData1 = '[Q]284{' + that.form.object_id + '$`' + meta[0] + '$`' + meta[1] + '$`' +
                  ...
                  + that.form.samples_proc + '$`' + that.form.expression + '$`' + that.form.item_category + '$`' + that.form.meta_desc +
                  '$`0$`'+that.form.in_entry + '$`' + that.form.in_mixed + '$`' + that.form.in_check + '$`' + that.form.belong+'$`'+this.form.template_id+'}|^2|^SYS'
                console.log(sData1)
                ...
                for(let i=1;i<=Number(that.form.max_samples);i++)
                {
                  ...
                  maxsamples = i.toString()
                	let sData = '[Q]284{' + that.form.object_id + '$`' + (Number(meta[0])*1000 + i).toString() + '$`' + meta[1] +'试样'+ maxsamples + '$`' +
                		that.form.group_id + '$`' + that.form.in_order + '$`' + that.form.in_record + '$`' + that.form.in_report + '$`' +
                    that.form.is_auto + '$`' + that.form.need_judge + '$`' + that.form.data_type +
                     ...
                     + that.form.samples_proc + '$`' + that.form.expression + '$`' + that.form.item_category + '$`' + that.form.meta_desc +
                		'$`' + re +'$`'+that.form.in_entry + '$`' + that.form.in_mixed + '$`' + that.form.in_check + '$`' + that.form.belong+'$`'+this.form.template_id+'}|^2|^SYS'
                	console.log(sData)
                  await that.ExecMetaAdd1(sData)
                }
                that.close_dialog(symbol)
              }
              else if(that.form.item_category === '普通项' && Number(that.form.max_samples)>1)
              {
                let sData1 = '[Q]284{' + that.form.object_id + '$`' + meta[0] + '$`' + meta[1] + '$`' +
                  that.form.group_id + '$`' + that.form.in_order + '$`' + that.form.in_record + '$`' + that.form.in_report + '$`' +
                  that.form.is_auto + '$`' + that.form.need_judge + '$`' + that.form.data_type + '$`' + that.form.input_type + '$`' + that.form.rounding + '$`' + that.form.scale +
                ...
                for(let i=1;i<=Number(that.form.max_samples);i++)
                {
                  let maxsamples
                  maxsamples = i.toString()
                	let sData = '[Q]284{' + that.form.object_id + '$`' + (Number(meta[0])*1000 + i).toString() + '$`' + meta[1] + maxsamples + '$`' +
                		that.form.group_id + '$`' + that.form.in_order + '$`' + that.form.in_record + '$`' + that.form.in_report + '$`' +
                    that.form.is_auto + '$`' + that.form.need_judge + '$`' + that.form.data_type +
                		'$`' + that.form.input_type + '$`' + that.form.rounding + '$`' + that.form.scale +
                		'$`' + that.form.metric + '$`' + that.form.metric_unit + '$`1$`1$`'
              ...
              else if(that.form.item_category === '指标项')
              {
                ...
                let sData2 = '[Q]284{' + that.form.object_id + '$`' + (Number(meta[0])*1000 + i) + '$`' + meta[1] +'标准值'+ '$`' +
                  ...
                  '$`' + that.form.metric + '$`' + that.form.metric_unit + '$`1$`1$`'
                  + that.form.samples_proc + '$`$`' + that.form.item_category + '$`' + that.form.meta_desc +
                  '$`' + re +'$`'+that.form.in_entry + '$`' + that.form.in_mixed + '$`' + that.form.in_check + '$`' + that.form.belong+'$`'+this.form.template_id+'}|^2|^SYS'
                console.log(sData2)
                await that.ExecMetaAdd(sData2,symbol)

              }
              else if(that.form.item_category === '替代项')
              {
                let i = 1
                let sData1 = '[Q]284{' + that.form.object_id + '$`' + meta[0] + '$`' + meta[1] + '$`' +
                  that.form.group_id + '$`' + that.form.in_order + '$`' + that.form.in_record + '$`' + that.form.in_report + '$`' +
                  that.form.is_auto + '$`' + that.form.need_judge + '$`' + that.form.data_type + '$`' + that.form.input_type + '$`' + that.form.rounding + '$`' + that.form.scale +
                  '$`' + that.form.metric + '$`' + that.form.metric_unit + '$`' + that.form.max_samples + '$`' + that.form.act_samples + '$`'
                  + that.form.samples_proc + '$`' + that.form.expression + '$`' + '指标项' + '$`' + that.form.meta_desc +
                  '$`0$`'+that.form.in_entry + '$`' + that.form.in_mixed + '$`' + that.form.in_check + '$`' + that.form.belong+'$`'+this.form.template_id+'}|^2|^SYS'
                console.log(sData1)
                let re = await that.ExecMetaAdd1(sData1)
                for(let k=0;k<Number(that.form.samples_number);k++)
                {
                  let sData1 = '[Q]284{' + that.form.object_id + '$`' + (Number(meta[0])+k).toString() + '$`' + meta[1]+(k+1).toString() + '$`' +
                    that.form.group_id + '$`' + that.form.in_order + '$`' + that.form.in_record + '$`' + that.form.in_report + '$`' +
                    that.form.is_auto + '$`' + that.form.need_judge + '$`' + that.form.data_type + '$`' + that.form.input_type + '$`' + that.form.rounding + '$`' + that.form.scale +
                    '$`' + that.form.metric + '$`' + that.form.metric_unit + '$`' + that.form.max_samples + '$`' + that.form.act_samples + '$`'
                    + that.form.samples_proc + '$`' + that.form.expression + '$`' + '指标项' + '$`' + that.form.meta_desc +
                    '$`' + re +'$`'+that.form.in_entry + '$`' + that.form.in_mixed + '$`' + that.form.in_check + '$`' + that.form.belong+'$`'+this.form.template_id+'}|^2|^SYS'
                  console.log(sData1)
                  await that.ExecMetaAdd1(sData1)
                  if(Number(that.form.max_samples)>1)
                  {
                    for(i=1;i<=Number(that.form.max_samples);i++)
                    {
                      let maxsamples
                      maxsamples = i.toString()
                      let sData = '[Q]284{' + that.form.object_id + '$`' + ((Number(meta[0])+k)*1000 + i).toString() + '$`' + meta[1]+(k+1).toString() +'试样'+ maxsamples + '$`' +
                        that.form.group_id + '$`' + that.form.in_order + '$`' + that.form.in_record + '$`' + that.form.in_report + '$`' +
                        that.form.is_auto + '$`' + that.form.need_judge + '$`' + that.form.data_type + '$`' + that.form.input_type + '$`' + that.form.rounding + '$`' + that.form.scale +
                        '$`' + that.form.metric + '$`' + that.form.metric_unit + '$`1$`1$`'
                        + that.form.samples_proc + '$`' + that.form.expression + '$`' + '指标项' + '$`' + that.form.meta_desc +
                  ...
                  let sData2 = '[Q]284{' + that.form.object_id + '$`' + ((Number(meta[0])+k)*1000 + i) + '$`' + meta[1]+(k+1).toString() +'标准值'+ '$`' +
                    that.form.group_id + '$`' + that.form.in_order + '$`0$`' + that.form.in_report + '$`' +
                    that.form.is_auto + '$`0$`' + that.form.data_type + '$`' + that.form.input_type + '$`' + that.form.rounding + '$`' + that.form.scale +
                    '$`' + that.form.metric + '$`' + that.form.metric_unit + '$`' + that.form.max_samples + '$`' + that.form.act_samples + '$`'
                    + that.form.samples_proc + '$`' + that.form.expression + '$`' + '指标项' + '$`' + that.form.meta_desc +
                  ...
                  let sData3 = '[Q]284{' + that.form.object_id + '$`' + ((Number(meta[0])+k)*1000 + (i+1)) + '$`' + meta[1]+(k+1).toString() +'名称'+ '$`' +
                    that.form.group_id + '$`' + that.form.in_order + '$`0$`' + that.form.in_report + '$`' +
                    that.form.is_auto + '$`0$`文本$`' + that.form.input_type + '$`' + that.form.rounding + '$`' + that.form.scale +
                    '$`' + that.form.metric + '$`' + that.form.metric_unit + '$`' + that.form.max_samples + '$`' + that.form.act_samples + '$`'
                    + that.form.samples_proc + '$`' + that.form.expression + '$`' + '指标项' + '$`' + that.form.meta_desc +
                    '$`' + re +'$`'+that.form.in_entry + '$`' + that.form.in_mixed + '$`' + that.form.in_check + '$`' + that.form.belong+'$`'+this.form.template_id+'}|^2|^SYS'
                  console.log(sData3)
                  await that.ExecMetaAdd1(sData3)
                }
                that.close_dialog(symbol)
              }
              else {
                let sData = '[Q]284{' + that.form.object_id + '$`' + meta[0] + '$`' + meta[1] + '$`' +
                  that.form.group_id + '$`' + that.form.in_order + '$`' + that.form.in_record + '$`' + that.form.in_report + '$`'
                  + that.form.is_auto + '$`' + that.form.need_judge + '$`' + that.form.data_type +
                  '$`' + that.form.input_type + '$`' + that.form.rounding + '$`' + that.form.scale +
                  '$`' + that.form.metric + '$`' + that.form.metric_unit + '$`' + that.form.max_samples + '$`' + that.form.act_samples + '$`'
                  ...
                  '$`0$`'+that.form.in_entry + '$`' + that.form.in_mixed + '$`' + that.form.in_check + '$`' + that.form.belong+'$`'+this.form.template_id+'}|^2|^SYS'
                console.log(sData)
                await that.ExecMetaAdd(sData,symbol)
              }
            } else {
            	return false
            }
      ...
      async ExecMetaAdd(sData,symbol) {
        let that = this
      ...
      async editsave() {
        ...
        if(ju)
        {
          this.$refs['form'].validate(async (valid) => {
          	if (valid) {
          		let sData = '[Q]482{' + that.form.id + '$`' + meta[0] + '$`' + meta[1] +
          			'$`' +
          			...
          			.samples_proc + '$`' + that.form.expression + '$`' + that.form.item_category + '$`' + that.form.meta_desc +
          			'$`'+that.form.in_entry + '$`' + that.form.in_mixed + '$`' + that.form.in_check + '$`' + that.form.belong+'}|^1|^SYS'
          		console.log(sData)
              const res = await ExecSql(sData)
              console.log(res)
              if (res > 0) {
