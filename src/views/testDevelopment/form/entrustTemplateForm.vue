<template>
	<div style="height:calc(78vh)">
		<el-form ref="form" :model="form" :rules="rules">
            ...
            <el-select v-model="form.template_name" style="width: 100%;" filterable remote placeholder="请输入关键词" v-else @change="templatechange"
...
<script>
	...
	export default {
      ...
      async getPersonList(query, page) {
      	...
      	let sData
      	sData = '[Q]479{%' + query + '%$`2$`' + ((Number(page)) * 10).toString() + '$`10}|^SYS'
      	const res = await QueryExec(sData)
      	const data = res.data
      ...
      displayold(row) {
      	...
      	that.form.template_json = row.template_json
        that.form.object_id = row.object_id
      	that.form.template_class = '2'
        that.form.tpl_class = '10'
        console.log(row)
        that.oldsymbol = true
      ...
      oldsave() {
				...
				this.$refs['form'].validate(async (valid) => {
					if (valid) {
						...
						if (res > 0) {
							let sData1 = '[Q]489{' + that.form.object_id + '$`' + res + '$`' + that.form.tpl_class +
								'}|^2|^SYS'
							console.log(sData1)
							const res1 = await ExecSql(sData1)
							if (res1 > 0) {
                let temp = {
                  "old_id":this.form.template_id,
                  "new_id":res
                }
                that.$refs['form'].resetFields()
                console.log(temp)
                that.$emit('addDictionary',temp)
							} else {
