		...
		<el-row>
			...
			<el-col :span="span.full">
				<el-form-item label="" :label-width="formLabelWidth" prop="">
					<el-alert title="提示" description="加载标准物质类型的前50条数据,如没有需要的请在下拉框中输入关键词进行搜索" type="info"
						:closable="false" center show-icon> </el-alert>
				</el-form-item>
			</el-col>
			<el-col :span="span.full">
				<el-form-item label="标准物质名称" :label-width="formLabelWidth" prop="mat_id">
					<el-select v-model="form.mat_id" style="width: 100%" :disabled="readonly"
						:multiple="select.mat_id.multiple" clearable filterable remote
						:remote-method="chemicalRemoteMethod" @change="chemicalChange">
						<el-option v-for="data in selectData.mat_id" :key="data.item_id"
	...
	export default {
		...
		methods: {
			...
			async init() {
				...
				switch (this.type) {
					...
					case 0:
						this.readonly = false
						this.form.device_id = ''
						this.select.mat_id.multiple = false
			...
			chemicalChange(value) {
				let data = this.selectData.mat_id
			...
			async GetChemicalId(searchKey) {
				let mat_id = this.form.mat_id
				let param_id = this.form.param_id
				let mat_id_typeof = (typeof mat_id).toLowerCase()
				if (isNumEx(mat_id)) {
					mat_id = Number(mat_id)
				}
				switch (mat_id_typeof) {
					case 'object':
						try {
							mat_id = mat_id.join(',')
						} catch (e) {
