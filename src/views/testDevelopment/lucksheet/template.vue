...
<script>
	...
	import improtLuckyExcel from 'luckyexcel'
	import {
		colToStr
	...
	export default {
        ...
        console.log(data)
			...
			isMultipleSelectChange(isMultipleSheet) {
				console.log('iframe isMultipleSelectChange', isMultipleSheet)
			...
			async getPostMessage(option) {
				...
				if (!messageListener || vue_that != this) {
					vue_that = this
					console.log('getPostMessage messageListener', messageListener)
					messageListener = function(e) {
						...
						console.log('iframe getPostMessage', data)
				...
				if (txt) {
					...
					let index = setInterval(() => {
						if (that.isHasLuckSheet) {
							...
							luckysheet.ManualPaste(txt, 0, 0, that.isCompressed, true, {
								sheet: {
									add_pos: 'start_before'
								}
							})
						}
					}, 500)
			...
			async Copy(rowRange, colRange, isMultipleSheet) {
				...
				console.log(content)
				if (content.code > 0) {
					this.postMessage('TemplateJson', content.data);
				} else {
					this.$notify({
						title: '提示',
			...
			error() {
				...
				function handleError(msg, url, l) {
					try {
						...
						// 	message: txt
						// });
						//that.$message.error(txt);
						that.postMessage('handleError', txt);
					} catch (e) {
						alert(e)
					}
					return true
