  ...
  <!-- 标准物质消耗定义右侧表格高度 -->
	<div>
		<el-table ref="branchInstitutions" v-loading="listLoading" :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip prop="name" label="标准物质名称" min-width="150px" align="center">
			</el-table-column>
			...
			<el-table-column show-overflow-tooltip label="操作" width="150px" align="center" fixed="right">
				...
				<template #default="{ row }">
					...
					<el-button @click="deleteMap(row)" class="danger" v-if="ishasPermission('delete')">删除
					</el-button>
	...
	export default {
		...
		methods: {
			...
			async init() {
				...
				switch (this.type) {
					case 10:
						this.height = 'calc(89.7%)'//标准物质消耗定义右侧表格高度优化 1920√ 1366出现滚动条
						this.columnHideList = []
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					10: {
						add: 'materialConsumptionAdd',
						edit: 'materialConsumptionEdit',
						delete: 'materialConsumptionDelete',
					},
				}
				switch (name) {
