	...
	<div class="table-container purchase equipment">
		...
		<el-container class="colors">
			<el-row :gutter="10">
				<el-col :span="6">
					<el-col :span="24">
						<el-input class="input-with-select" placeholder="请输入参数名称" v-model="search.param">
							<template #prepend>
								<el-select v-model="tpl_class" style="width: 120px">
					...
					<el-col :span="24">
						<el-table ref="tableSort" :data="list" border :element-loading-text="elementLoadingText"
							:height="height" row-key="param_id" highlight-current-row
							:tree-props="{ children: 'children', hasChildren: 'hasChildren' }" lazy
							tooltip-effect="dark" :load="fetchDataEx" @row-click="handleRowClick">
							<el-table-column show-overflow-tooltip prop="param" label="参数名称" min-width="80%"
								align="left">
							</el-table-column>
	...
	export default {
      ...
      addmap() {
        ...
        that.listsumbit.forEach(async(item, index) => {
          if(item.state === 'I')
          {
            let sData = '[Q]452{'+that.form.param_id+'$`'+that.form.tpl_class+'$`'+item.meta_id+'$`'+item.cell+'$`'+this.form.temp_id+'}|^2|^SYS'
          ...
          {
            let sData = '[Q]478{'+that.form.param_id+'$`'+that.form.tpl_class+'$`'+item.meta_id+'$`'+item.cell+'$`'+this.form.temp_id+'}|^1|^SYS'
            const res = await ExecSql(sData)
            if(res>0)
            {
          ...
          {
            let sData = '[Q]477{'+that.form.param_id+'$`'+that.form.tpl_class+'$`'+item.meta_id+'$`'+this.form.temp_id+'}|^1|^SYS'
            const res = await ExecSql(sData)
            if(res>0)
            {
      ...
      prePage() {
			...
			async fetchDataChild(row) {
				...
				if (row) {
					let sData = '[Q]287{' + row.param_id + '$`' + ((Number(this.queryForm.pageNo) - 1) * Number(this
        ...
        let index = setTimeout(() => {
          ...
          res1.data.forEach((item, index) => {
			...
			async fetchDataChildFile(row) {
				this.listChildLoading = true
				if (row) {
					let sDataFile = '[Q]437{' + row.param_id + '$`' + ((Number(this.queryFormFile.pageNo) - 1) *
			...
			async fetchDataEx(row, treeNode, resolve) {
				...
				if (row) {
					...
					sData = '[Q]251{' + form.parent_id + '$`%' + this.search.param + '%$`' + this.search.enable +
						'$`' + this.search.enable_time_start + '$`' + this.search.enable_time_end + '}|^SYS'

				} else {
					form.parent_id = 0
					sData = '[Q]250{' + form.parent_id + '$`%' + this.search.param + '%$`' + this.search.enable +
						'$`' + this.search.enable_time_start +
						'$`' + this.search.enable_time_end + '$`' + ((Number(this.queryForm.pageNo) - 1) * Number(this
							.queryForm.pageSize)).toString() + '$`' + this.queryForm.pageSize + '}|^SYS'
				...
				if (row) {
					...
					const data = TreeDataDealEx(res.data, 0, 'param_id', 'parent_id', 'hasChildren')
					console.log(data)
					this.list = data
				}
				// } catch (e) {
