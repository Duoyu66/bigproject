	...
	<div class="table-container purchase equipment">
		...
		<el-container class="colors">
			<el-row :gutter="10">
				<el-col :span="6">
					<el-col :span="24">
						<el-input class="input-with-select" placeholder="请输入试验名称" v-model="search.object_name" style="padding-bottom: 10px;">
							<template #append>
              ...
              <el-select v-model="search.template_name" style="" filterable placeholder="请选择模板">
              	<el-option v-for="item in listChildFile" :key="item.template_id" :label="item.template_name"
        ...
        let index = setTimeout(() => {
          ...
          res1.data.forEach((item, index) => {
			...
			async fetchDataChildFile(row) {
				this.listChildLoading = true
				if (row) {
					let sDataFile = '[Q]490{' + row.object_id + '$`' + ((Number(this.queryFormFile.pageNo) - 1) *
