...
<!--            <el-form ref="form" :model="form">
            ...
            >
              ...
              <el-table-column
                ...
                prop="storage_place"
              ></el-table-column>
              <el-table-column show-overflow-tooltip label="是否危险品">
                <template #default="{ row }">
                  <span v-if="row.danger == 1">是</span>
                  <span v-else>否</span>
                </template>
              </el-table-column>
...
import TableEdit5 from '../components/contractChemicalManagement.vue'
import TableEdit9 from '../components/Contact1.vue'
import ContactForm from '../form/ContractChemicalForm.vue'
  ...
  methods: {
    ...
    transferTable() {
      this.$baseConfirm('是否跳转到合同台账?如果要继续添加请点击取消。', null, async () => {
        this.$router.push('/contractChemicalManage?tabs=1')
        this.fetchData()
      })
    },
    type_change() {
    ...
    async fetchDataChild(val)
    {
      ...
      if(val && val.customer_id)
      {
        ...
        let sDataReceivable = '[Q]379{'+this.contract_id+'$`'+((Number(this.queryFormReceivable.pageNo)-1)*Number(this.queryFormReceivable.pageSize)).toString()+'$`'+this.queryFormReceivable.pageSize+'}|^SYS'
