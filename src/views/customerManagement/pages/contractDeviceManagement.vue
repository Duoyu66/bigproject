		...
		<el-tabs type="border-card" :value="this.$route.query.tabs ? this.$route.query.tabs : 1" class="box_center">
			<el-tab-pane label="合同登记" class="center">
				<div class="usage">
					...
					<div>
						<el-button class="purchase_btn" type="primary" @click="ContactFormAdd()">
							确 定
						</el-button>
					</div>
				</div>
              ...
              <el-option
                ...
                :value="item.item_id">
					...
					<vab-query-form-right-panel style="width: 100%; display: flex; justify-content: flex-end">
						<div class="block">
							<el-input style="border-radius: 30px" v-model="search.contract_no" placeholder="以合同编号搜索"
								prefix-icon="el-icon-search" />
						</div>
						<div class="block">
							<el-input style="border-radius: 30px" v-model="search.contract_name" placeholder="以合同名称搜索"
								prefix-icon="el-icon-search" />
						</div>
						<div class="block">
							<el-select v-model="search.sponsor_org" style="width: 100%" filterable placeholder="请选择">
								<el-option v-for="item in sponsor_org_options" :key="item.item_name"
				...
				<el-table ref="tableSort" v-loading="listLoading" :data="listContract" border
					:element-loading-text="elementLoadingText" highlight-current-row :height="table.tableHeightMain"
					@selection-change="setSelectRows" @sort-change="tableSortChange"
					:header-cell-style="{ background: '#e8f0ff' }" @row-click="handleSetRows">
					<el-table-column show-overflow-tooltip label="所属部门" prop="org_name" width="150px"></el-table-column>
					<el-table-column show-overflow-tooltip label="合同编号" prop="contract_no" width="150px">
					</el-table-column>
					<el-table-column show-overflow-tooltip prop="contract_name" label="合同名称" width="150px">
					</el-table-column>
					<el-table-column show-overflow-tooltip label="客户名称" prop="customer_name" width="100px">
					</el-table-column>
					<el-table-column show-overflow-tooltip prop="contract_signdate" label="签订日期" width="200px">
					</el-table-column>
					<el-table-column show-overflow-tooltip label="合同采购额" prop="contract_money" width="100px">
					</el-table-column>
					...
					<el-table-column show-overflow-tooltip prop="sum1" label="应付金额" width="100px"></el-table-column>
					<el-table-column show-overflow-tooltip label="已付金额" prop="sum2" width="100px"></el-table-column>
					<el-table-column show-overflow-tooltip label="剩余应付" prop="sum5" width="100px"></el-table-column>
					<el-table-column show-overflow-tooltip label="发票已开" prop="sum3" width="100px"></el-table-column>
					<el-table-column show-overflow-tooltip label="发票未付金额" prop="sum4" width="120px"></el-table-column>
                  ...
                  <el-button @click="handleContactAdd(row)">添加联系人</el-button>
									...
									<el-button @click="handleAddPurchase(row,'0')">添加设备购入</el-button>
				...
				<el-pagination :background="background" :current-page="queryFormMain.pageNo" :layout="layoutMain"
					:page-size="queryFormMain.pageSize" :total="totalMain" @current-change="handleCurrentChangeMain"
					@size-change="handleSizeChangeMain"></el-pagination>
				<el-tabs style="margin-top: 15px">
					<el-tab-pane label="发票">
						...
						<el-table ref="tableSort" v-loading="listChildLoading" :data="listReceipt" border
							:element-loading-text="elementLoadingText" highlight-current-row
							:height="table.tableHeightChild1" @selection-change="setSelectRows"
							@sort-change="tableSortChange" :header-cell-style="{ background: '#e8f0ff' }">
							<el-table-column show-overflow-tooltip label="发票号" prop="invoice_no"></el-table-column>
							<el-table-column show-overflow-tooltip prop="amount" label="金额(万)"></el-table-column>
							<el-table-column show-overflow-tooltip label="税率" prop="tax_ratio"></el-table-column>
							<el-table-column show-overflow-tooltip label="开票日期" prop="invoice_date"></el-table-column>
							<el-table-column show-overflow-tooltip prop="operator" label="开票人"></el-table-column>
							<el-table-column show-overflow-tooltip label="发票类型" prop="type"></el-table-column>
							...
							<el-table-column show-overflow-tooltip label="操作" width="120px">
								<template #default="{ row }">
									<el-button type="primary" class="normal" @click="handleReceiptEdit(row)">
										修改
									</el-button>
						...
						<el-pagination :background="background" :current-page="queryFormReceipt.pageNo"
							:layout="layoutReceipt" :page-size="queryFormReceipt.pageSize" :total="totalReceipt"
							@current-change="handleCurrentChangeReceipt" @size-change="handleSizeChangeReceipt">
						</el-pagination>
					</el-tab-pane>
					<el-tab-pane label="设备采购台账">
						<!--            <el-form ref="form" :model="form">
              <el-form-item label="应收款合计:" prop="">
              	<span>{{form.sumneedmoney}}</span>
              </el-form-item>
            </el-form> -->
						<el-table ref="tableSort" v-loading="listChildLoading" :data="listReceivable" border
							:element-loading-text="elementLoadingText" highlight-current-row
							:height="table.tableHeightChild" @selection-change="setSelectRows"
							@sort-change="tableSortChange" :header-cell-style="{ background: '#e8f0ff' }">
							<el-table-column
							...
							<el-table-column show-overflow-tooltip label="操作" fixed="right" width="80px">
								<template #default="{ row }">
									<el-dropdown trigger="click" placement="left-start">
										...
										<el-dropdown-menu slot="dropdown" class="more">
											<el-button @click="handleAdd11(row)">修改
											</el-button>
											<el-button @click="handleDeviceDelete(row)">删除
											</el-button>
											<el-button v-if="row.is_accept === '0'" @click="handleAccept(row)">验收设备
											</el-button>
										</el-dropdown-menu>
									</el-dropdown>
								</template>
						...
						<el-pagination :background="background" :current-page="queryFormReceivable.pageNo"
							:layout="layoutReceivable" :page-size="queryFormReceivable.pageSize"
							:total="totalReceivable" @current-change="handleCurrentChangeReceivable"
							@size-change="handleSizeChangeReceivable"></el-pagination>
					</el-tab-pane>
					<el-tab-pane label="已付款历史">
						...
						<el-table ref="tableSort" v-loading="listChildLoading" :data="listReceived" border
							:element-loading-text="elementLoadingText" highlight-current-row
							:height="table.tableHeightChild1" @selection-change="setSelectRows"
							@sort-change="tableSortChange" :header-cell-style="{ background: '#e8f0ff' }">
							<el-table-column show-overflow-tooltip label="委托编号" prop="data_id">
								<template slot-scope="scope">
									<span v-if="scope.row.data_id===''">无</span>
									<span v-else>{{scope.row.data_id}}</span>
								</template>
							</el-table-column>
							<el-table-column show-overflow-tooltip label="付款时间" prop="pay_time"></el-table-column>
							<el-table-column show-overflow-tooltip label="付款人" prop="payee"></el-table-column>
							<el-table-column show-overflow-tooltip prop="amount" label="金额"></el-table-column>
							<el-table-column show-overflow-tooltip label="付款方式" prop="channel"></el-table-column>
							<el-table-column show-overflow-tooltip label="事由" prop="memo"></el-table-column>
							<el-table-column show-overflow-tooltip label="操作" width="120px">
								<template #default="{ row }">
									<el-button type="primary" class="normal" @click="handleReceivedEdit(row)">
										修改
									</el-button>
						...
						<el-pagination :background="background" :current-page="queryFormReceived.pageNo"
							:layout="layoutReceived" :page-size="queryFormReceived.pageSize" :total="totalReceived"
							@current-change="handleCurrentChangeReceived" @size-change="handleSizeChangeReceived">
						</el-pagination>
					</el-tab-pane>
					<el-tab-pane label="快递">
						<el-table ref="tableSort" v-loading="listChildLoading" :data="listPost" border
							:element-loading-text="elementLoadingText" highlight-current-row
							:height="table.tableHeightChild" @selection-change="setSelectRows"
							@sort-change="tableSortChange" :header-cell-style="{ background: '#e8f0ff' }">
							<el-table-column show-overflow-tooltip label="快递公司" prop="express_company">
							</el-table-column>
							<el-table-column show-overflow-tooltip label="快递单号" prop="track_id"></el-table-column>
							<el-table-column show-overflow-tooltip prop="type" label="类型"></el-table-column>
							<el-table-column show-overflow-tooltip label="发件人" prop="sender"></el-table-column>
							<el-table-column show-overflow-tooltip label="收件人" prop="receiver"></el-table-column>
							<el-table-column show-overflow-tooltip prop="telephone" label="收件电话"></el-table-column>
							<el-table-column show-overflow-tooltip label="收件地址" prop="address"></el-table-column>
							<!--              <el-table-column
                show-overflow-tooltip
                label="客户名称"
                prop="customer_id"
              ></el-table-column> -->
							<el-table-column show-overflow-tooltip label="金额" prop="money"></el-table-column>
							<el-table-column show-overflow-tooltip label="操作" width="120px">
								<template #default="{ row }">
									<el-button type="primary" class="normal" @click="handlePostEdit(row)">
										修改
									</el-button>
						...
						<el-pagination :background="background" :current-page="queryFormPost.pageNo"
							:layout="layoutPost" :page-size="queryFormPost.pageSize" :total="totalPost"
							@current-change="handleCurrentChangePost" @size-change="handleSizeChangePost">
						</el-pagination>
					</el-tab-pane>
					<el-tab-pane label="联系人列表">
						<el-table ref="tableSort" v-loading="listChildLoading" :data="listContacts" border
							:element-loading-text="elementLoadingText" highlight-current-row
							:height="table.tableHeightChild" @selection-change="setSelectRows"
							@sort-change="tableSortChange" :header-cell-style="{ background: '#e8f0ff' }">
							<el-table-column show-overflow-tooltip label="联系人姓名" prop="name"></el-table-column>
							<el-table-column show-overflow-tooltip prop="mobile" label="联系电话"></el-table-column>
							<el-table-column show-overflow-tooltip label="联系人职务" prop="duty"></el-table-column>
							<el-table-column show-overflow-tooltip label="QQ" prop="QQ"></el-table-column>
							<el-table-column show-overflow-tooltip label="微信" prop="WeChat"></el-table-column>
							<el-table-column show-overflow-tooltip label="电子邮箱" prop="email"></el-table-column>
							<el-table-column show-overflow-tooltip label="操作" width="120px">
								<template #default="{ row }">
									<el-button type="primary" class="normal" @click="handleContactEdit(row,'0')">
										修改
									</el-button>
						...
						<el-pagination :background="background" :current-page="queryFormContacts.pageNo"
							:layout="layoutContacts" :page-size="queryFormContacts.pageSize" :total="totalContacts"
							@current-change="handleCurrentChangeContacts" @size-change="handleSizeChangeContacts">
						</el-pagination>
					</el-tab-pane>
					<el-tab-pane label="联系记录">
						<el-table ref="tableSort" v-loading="listChildLoading" :data="listContactRecord" border
							:element-loading-text="elementLoadingText" highlight-current-row
							:height="table.tableHeightChild" @selection-change="setSelectRows"
							@sort-change="tableSortChange" :header-cell-style="{ background: '#e8f0ff' }">
							<el-table-column show-overflow-tooltip label="联系原因" prop="reason"></el-table-column>
							<el-table-column show-overflow-tooltip label="联系人" prop="name"></el-table-column>
							<el-table-column show-overflow-tooltip label="联系日期" prop="create_time1"></el-table-column>
<!-- 							<el-table-column show-overflow-tooltip label="下次联系日期" prop="next_time1"></el-table-column> -->
							<el-table-column show-overflow-tooltip label="实施人" prop="user_name"></el-table-column>
							<el-table-column show-overflow-tooltip label="备注" prop="remarks"></el-table-column>
							<el-table-column show-overflow-tooltip label="操作" width="120px">
								<template #default="{ row }">
									<el-button type="primary" class="normal" @click="handleCHEdit(row)">
										修改
									</el-button>
						...
						<el-pagination :background="background" :current-page="queryFormContactRecord.pageNo"
							:layout="layoutContactRecord" :page-size="queryFormContactRecord.pageSize"
							:total="totalContactRecord" @current-change="handleCurrentChangeContactRecord"
							@size-change="handleSizeChangeContactRecord"></el-pagination>
					</el-tab-pane>
		...
		<table-edit11 ref="edit11" @fetchDataChild="fetchDataChild"></table-edit11>
...
<script>
	...
	import TableEdit5 from '../components/contractDeviceManagement.vue'
	import TableEdit9 from '../components/Contact1.vue'
	import ContactForm from '../form/ContractDeviceForm.vue'
	import preview from '@/components/fileAbout/preview'
	import TableEdit11 from '../../deviceManagement/components/equipmentPurchase.vue'
		...
		methods: {
			...
			transferTable() {
				this.$baseConfirm('是否跳转到合同台账?如果要继续添加请点击取消。', null, async () => {
					this.$router.push('/contractDeviceManagement?tabs=1')
					this.fetchData()
				})
			},
			type_change() {
			...
			handleAddPurchase(row) {
				this.$refs['edit11'].showEdit(row, '0')
			},
        ...
        this.$refs['ContactAdd'].showEdit(row)
			...
			async fetchDataChild(val) {
				...
				if (val && val.customer_id) {
					this.listrow = val
					let sDataReceipt = '[Q]213{' + val.contract_id + '$`' + ((Number(this.queryFormReceipt.pageNo) -
							1) * Number(this.queryFormReceipt.pageSize)).toString() + '$`' + this.queryFormReceipt
						.pageSize + '}|^SYS'
					...
					let sDataReceivable = '[Q]639{' +val.contract_id+'$`%%$`%%$`'+ ((Number(this.queryFormReceivable
							.pageNo) - 1) * Number(this.queryFormReceivable.pageSize)).toString() + '$`' + this
						.queryFormReceivable.pageSize + '}|^SYS'
					...
					let sDataReceived = '[Q]226{' + val.contract_id + '$`' + ((Number(this.queryFormReceived.pageNo) -
							1) * Number(this.queryFormReceived.pageSize)).toString() + '$`' + this.queryFormReceived
						.pageSize + '}|^SYS'
					...
					let sDataPost = '[Q]231{' + val.contract_id + '$`' + ((Number(this.queryFormPost.pageNo) - 1) *
							Number(this.queryFormPost.pageSize)).toString() + '$`' + this.queryFormPost.pageSize +
						'}|^SYS'
					...
					let sDataContacts = '[Q]214{' + val.customer_id + '$`' + ((Number(this.queryFormContacts.pageNo) -
							1) * Number(this.queryFormContacts.pageSize)).toString() + '$`' + this.queryFormContacts
						.pageSize + '}|^SYS'
					...
					let sDataContactRecord = '[Q]236{' + val.contract_id + '$`0$`' + ((Number(this.queryFormContactRecord
							.pageNo) - 1) * Number(this.queryFormContactRecord.pageSize)).toString() + '$`' + this
						.queryFormContactRecord.pageSize + '}|^SYS'
					...
					let sDataEntrustContract = '[Q]301{' + this.current_org + '$`' + val.contract_id + '$`' + ((Number(
						this.queryFormContractRecord.pageNo) - 1) * Number(this.queryFormContractRecord
						.pageSize)).toString() + '$`' + this.queryFormContractRecord.pageSize + '}|^SYS'
					...
					let sDataFile = '[Q]311{' + val.contract_id + '$`0$`' + ((Number(this.queryFormFile.pageNo) - 1) *
							Number(this.queryFormFile.pageSize)).toString() + '$`' + this.queryFormContactRecord
						.pageSize + '}|^SYS'
			...
			async fetchData() {
				...
				let sData = '[Q]212{'+this.current_org+'$`%'+this.search.contract_no+'%$`%'+this.search.contract_name
        +'%$`'+this.search.year+'$`'+this.search.contract_state+'$`'
				+this.search.sponsor_org+'$`'+this.form.contract_type+'$`'
        +((Number(this.queryFormMain.pageNo)-1)*Number(this.queryFormMain.pageSize)).toString()+'$`'+this.queryFormMain.pageSize+'}|^SYS'
