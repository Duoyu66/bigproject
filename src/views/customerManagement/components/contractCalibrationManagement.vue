...
<script>
 ...
 import ContractForm from '../form/ContractCalibrationForm.vue'
 export default {
   name: 'TableEdit8',
   components: {
   	ContractForm,
    ...
    methods: {
      showEdit(row, symbol) {
        if (!row && !symbol) {
          ...
          setTimeout(()=>{
            this.$refs['ContractForm'].display(this.Contact)
          },10)
        }
        else if(symbol === '1'){
          this.title = '修改合同'
          ...
          this.symbol = symbol
          setTimeout(()=>{
            this.$refs['ContractForm'].displayEdit(this.Contact)
          },10)
        }
        else if(symbol === '2'){
          this.title = '合同详情'
          ...
          this.symbol = symbol
          setTimeout(()=>{
            this.$refs['ContractForm'].displayadd(this.Contact)
          },10)
        }
        else if(symbol === '4'){
          this.title = '编辑委外合同'
          ...
          this.symbol = symbol
          setTimeout(()=>{
            this.$refs['ContractForm'].displayoutedit(this.Contact)
          },10)
        }
