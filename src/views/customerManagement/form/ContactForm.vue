<template>
    ...
    <el-form :model="form" :rules="rules" ref="form" v-loading="formloading">
      <el-form-item label="客户名称" :label-width="formLabelWidth">
        <el-select v-model="form.customer_id" v-if='displaymanage' style="width: 100%" filterable placeholder="请选择客户名称">
          <el-option
      	...
      	<el-input v-model="form.customer_name" style="width: 100%;" v-else disabled></el-input>
      </el-form-item>
      <el-form-item label="联系人姓名" prop="name" :label-width="formLabelWidth">
      	<el-input v-model="form.name" style="width: 100%;" maxlength="16"></el-input>
      </el-form-item>
    	<el-form-item label="联系人职务" prop="duty" :label-width="formLabelWidth">
    		<el-input v-model="form.duty" style="width: 100%;" type="textarea" maxlength="32"></el-input>
    	</el-form-item>
      ...
      <el-form-item label="qq" prop="QQ" :label-width="formLabelWidth">
      	<el-input v-model="form.QQ" style="width: 100%;" maxlength="24"></el-input>
      </el-form-item>
  ...
  import contractimageupload from '@/components/Upload/upload'
    ...
    computed: {
      ...
      async fetchData() {
        ...
        let sData = '[Q]341{'+this.current_org+'}|^SYS'
      ...
      save() {
        let that=this
      	that.$refs['form'].validate(async (valid) => {
      		if (valid) {
            let sData = '[Q]215{'+that.form.customer_id+'$`'+that.form.name+'$`'+that.form.duty+'$`'
      ...
      editsave() {
        let that=this
      	that.$refs['form'].validate(async (valid) => {
      		if (valid) {
            let sData = '[Q]217{'+that.form.name+'$`'+that.form.duty+'$`'
            +that.form.mobile+'$`'+that.form.email+'$`'+that.form.QQ+'$`'+that.form.WeChat+'$`'
