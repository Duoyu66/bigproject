...
<!--        <el-form-item label="客户名称" prop="customer_name" :label-width="formLabelWidth" >
        ...
        <el-form-item label="项目负责人" prop="contract_name" :label-width="formLabelWidth" >
          <el-select v-model="form.project_responsible" style="width: 100%"  filterable placeholder="请选择项目负责人">
            <el-option
              v-for="item in project_responsible_options"
              :key="item.person_id"
  ...
  const simulateAsyncOperation = fn => {
    ...
    components: {
      ...
      var validate_discount = (rule, value, callback) => {
        ...
        readonly: false,
        rules: {
          plan_amount: [{ validator: validate_receipt_cost, trigger: 'blur' }],
    ...
    computed: {
      ...
      async fetchData() {
        ...
        let sData = '[Q]849{'+this.current_org+'}|^SYS'
        const res = await QueryExec(sData)
        console.log(sData,res)
        this.project_responsible_options = res.data
        setTimeout(() => {
          this.selectloading = false
        }, 500)
      },
      save() {
        ...
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            let sData = '[Q]845{'+that.current_org+'$`'+that.form.contract_id+'$`'+that.form.plan_amount+'$`'+that.form.protect_amount+'}|^2|^SYS'
            console.log(sData)
      ...
      editsave() {
        ...
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            let sData = '[Q]850{'+that.form.project_responsible+'$`'+that.form.contract_id+'}|^1|^SYS'
            console.log(sData)
            const res = await ExecSql(sData)
            if(res>0)
            {
              that.$baseMessage('项目负责人关联成功', 'success')
              that.$refs['form'].resetFields()
              that.$emit('close_comp')
            }
            else
            {
              that.$baseMessage('项目负责人关联失败', 'error')
            }
          } else {
            return false
          }
