...
<!--        <el-form-item label="收款人" prop="payee" :label-width="formLabelWidth">
        ...
        <el-form-item label="单价(万)" prop="unitprice" :label-width="formLabelWidth">
        ...
        <el-form-item label="总金额(万)" prop="amount" :label-width="formLabelWidth">
    ...
    components: {
      ...
      var validate_discount = (rule, value, callback) => {
        ...
        rules: {
          ...
          unitprice: [{ required: true, trigger: 'blur', message: '请输入金额' },{ validator: validate_receipt_cost, trigger: 'blur' }],
        },
    ...
    computed: {
      ...
      save() {
        ...
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            let sData = '[Q]403{'+that.current_org+'$`'+that.form.customer_id+'$`'+that.form.contract_id+'$`'
            +that.form.amount+'$`'+that.form.discount+'$`'+that.form.charged_amt+'$`'+that.form.pay_channel+'$`1$`'
            +that.form.object_id+'$`0$`'+that.form.count+'$`'+that.form.unitprice+'$`'+that.form.receivable_time+'}|^2|^SYS'
            console.log(sData)
      ...
      editsave() {
        ...
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            let sData = '[Q]406{'+that.form.amount+'$`'+that.form.discount+'$`'+that.form.charged_amt+'$`'+that.form.pay_channel+'$`1$`'
            +that.form.object_id+'$`0$`'+that.form.count+'$`'+that.form.unitprice+'$`'+that.form.receivable_time+'$`'+that.form.receivable_id+'}|^1|^SYS'
            console.log(sData)
            const res = await ExecSql(sData)
            if(res>0 && that.form.receivable_id)
            {
              that.$baseMessage('收款修改成功', 'success')
              that.$refs['form'].resetFields()
              that.$emit('close_comp')
            }
            else
            {
              that.$baseMessage('收款修改失败', 'error')
            }
          } else {
            return false
          }
