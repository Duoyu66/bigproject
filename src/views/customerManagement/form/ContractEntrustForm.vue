<template>
	<span class="purchasefrom">
      <el-form ref="form" :model="form" :rules="rules" :label-width="formLabelWidth" v-loading="selectloading">
        ...
        <el-col :span="span" >
          	<el-form-item label="所属合同" prop="customer_name">
              <el-select v-model="form.parent_id" v-if='displaycontractoption' style="width: 100%" filterable placeholder="请选择所属合同">
                <el-option
          ...
          <el-col :span="span" >
            	<el-form-item label="受托单位" prop="customer_name">
            		<el-select v-model="form.customer_name" v-if='displaycustomername' style="width: 100%" filterable placeholder="请选择受托单位">
            		  <el-option
            		...
            		<el-input v-model="form.customer_name" v-else style="width: 100%" maxlength="20" disabled></el-input>
            	</el-form-item>
            </el-col>
<!-- 		  <el-col :span="span">
...
<script>
  ...
  import contractimageupload from '@/components/Upload/upload'
    ...
    computed: {
      ...
      displayEntrust() {
        this.selectloading = true
        this.displaycontractoption = true
        this.form.parent_id = ''
        ...
        let that=this
      	that.$refs['form'].validate(async (valid) => {
            ...
            that.fileUpload(async function(status) {
              ...
              +that.form.contract_type+'$`'+that.form.parent_id+'}|^2|^SYS'
              console.log(sData)
        ...
        if(that.form.contract_end === null)
        {
      	...
      	that.$refs['form'].validate(async (valid) => {
            ...
            that.fileUpload(async function(status) {
              ...
              +that.form.administrative+'$`'+that.form.operator_people+'$`'+that.form.contract_from+'$`'
              +that.form.project_simple+'$`'+that.form.work_content+'$`'+that.form.regard+'$`'+that.form.contract_filename+
              '$`'+that.form.contract_id+'$`'+that.form.customer_id+'$`'+that.form.parent_id+'}|^1|^SYS'
              console.log(sData)
              const res = await ExecSql(sData)
              if(res>0)
              {
      ...
      async fetchData() {
        ...
        let sData = '[Q]290{'+this.current_org+'$`1}|^SYS'
