      ...
      <el-form ref="form" :model="form" :rules="rules" :label-width="formLabelWidth" v-loading="selectloading">
        ...
        <el-col :span="span" >
          	<el-form-item :label="customer_label" prop="customer_name">
          		<el-select v-model="form.customer_name" v-if='isContractAdd' style="width: 100%" filterable placeholder="请选择委托单位">
          		  <el-option
              ...
              <el-select v-model="form.parent_id" v-else-if='isEntrustContractAdd' style="width: 100%" filterable placeholder="请选择所属合同">
                <el-option
          		...
          		<el-input v-model="form.customer_name" v-else-if='isCustomerContractAdd' style="width: 100%" disabled></el-input>
              <el-input v-model="form.parent_name" v-else style="width: 100%" disabled></el-input>
          	</el-form-item>
          </el-col>
<!-- 		  <el-col :span="span">
		  ...
		  <el-col :span="span">
		  	<el-form-item label="工程名称" prop="project_name">
		  		<el-input v-model="form.project_name" style="width: 100%" maxlength="64"></el-input>
		  	</el-form-item>
		  </el-col>
		  <el-col :span="span">
		  	<el-form-item label="承揽单编号" prop="projectcontract_no">
		  		<el-input v-model="form.projectcontract_no" style="width: 100%" maxlength="32"></el-input>
		  	</el-form-item>
		  </el-col>
        	...
        	<el-col :span="span">
        		<el-form-item label="合同编号" prop="contract_no">
        			<el-input v-model="form.contract_no" style="width: 100%" maxlength="32"></el-input>
        		</el-form-item>
        	</el-col>
<!--       	<el-col :span="span">
        	...
        	<el-col :span="span">
        		<el-form-item label="合同名称" prop="contract_name">
        			<el-input v-model="form.contract_name" style="width: 100%" maxlength="128"></el-input>
        		</el-form-item>
        	</el-col>
			<el-col :span="span">
				<el-form-item label="合同金额(万)" prop="contract_money">
...
<!-- 			<el-col :span="span">
			...
			<el-col :span="span">
				<el-form-item label="执行期限(始)" prop="contract_start">
					<el-date-picker
...
<!--        	<el-col :span="span">
		  ...
		  <el-col :span="span">
        	...
        	<el-col :span="span">
        		<el-form-item label="合同来源" prop="contract_from">
        			<el-input v-model="form.contract_from" style="width: 100%" maxlength="16"></el-input>
        		</el-form-item>
        	</el-col>
          <el-col :span="span">
          	<el-form-item label="合同状态" prop="contract_state">
          		<el-select v-model="form.contract_state" style="width: 100%" filterable placeholder="请选择">
          		  <el-option
          ...
          <el-col :span="span">
          	<el-form-item label="质保金(万)" prop="warranty_bond">
          		<el-input v-model="form.warranty_bond" style="width: 100%" maxlength="16"></el-input>
          	</el-form-item>
          </el-col>
<!--      <el-col :span="span">
...
<!--        	<el-col :span="span">
			...
			<el-col :span="24">
				<el-form-item label="规模简述" prop="project_simple">
 ...
 <!--         <el-col :span="span">
          ...
          <el-col :span="24">
            <el-form-item label="合同扫描件" prop="contract_filename">
  ...
  import contractimageupload from '@/components/Upload/upload'
    ...
    computed: {
      ...
      var validate_contract_signdate = (rule, value, callback) => {
        ...
        if (signdate>returndate && this.form.contract_signdate!=='' && value!=='') {
          callback(new Error('签订日期应小于返回日期'));
        } else {
          callback();
        }
      };
      var validate_contract_end = (rule, value, callback) => {
        let startdate = new Date(this.form.contract_start)
        let enddate = new Date(value)
        if (startdate>enddate  && value!=='' && this.form.contract_start!=='') {
      ...
      var validate_contract_start = (rule, value, callback) => {
        ...
        let enddate = new Date(value)
        if (startdate>enddate && this.form.contract_return!=='' && value!=='') {
        ...
        isContractAdd: true,
        isEntrustContractAdd: false,
        isCustomerContractAdd: false,
				form: {
					contract_id:'',
          customer_name: '',
					customer_id:'',
        ...
        rules: {
          customer_name: [{ required: true, trigger: 'change', message: '请选择委托单位' }],
          contract_state: [{ required: true, trigger: 'change', message: '请选择合同状态' }],
          ...
          contract_end: [{ validator: validate_contract_end, trigger: 'blur'}],
          warranty_bond: [{ validator: validate_contract_money, trigger: 'blur'}],
        },
    ...
    computed: {
		...
		methods: {
      //客户台账页面添加合同
      displayCustomerContractAdd(item) {
        console.log(item)
        this.selectloading = true
        this.isContractAdd = false
        this.isEntrustContractAdd = false
        this.isCustomerContractAdd = true
        this.form.customer_name=item.customer_name
        this.form.customer_id=item.customer_id
        this.rules.customer_name = []
        setTimeout(() => {
          this.selectloading = false
        }, 500)
      },
      //委外合同添加
      displayEntrustadd(item) {
        this.selectloading = true
        this.isContractAdd = false
        this.isEntrustContractAdd = true
        this.isCustomerContractAdd = false
        console.log(item)
      ...
      //合同台账页面编辑
      displayContractEdit(item) {
        this.selectloading = true
        this.isContractAdd = true
        this.isEntrustContractAdd = false
        this.isCustomerContractAdd = false
        console.log(item)
        ...
        let that=this
      	that.$refs['form'].validate(async (valid) => {
            ...
            that.fileUpload(async function(status) {
              if(that.isContractAdd === true)
              {
                that.form.customer_id = that.form.customer_name
              }
              console.log(that.form)
              let sData = '[Q]210{'+that.current_org+'$`'+that.form.customer_id+'$`'+that.form.org_entrust+'$`'
              ...
              +that.form.project_simple+'$`'+that.form.work_content+'$`'+that.form.regard+'$`'+that.form.contract_filename+'$`'
              +that.form.contract_type+'$`'+that.form.parent_id+'$`'+that.form.contract_state+'$`'+that.form.warranty_bond+'}|^2|^SYS'
              console.log(sData)
      ...
      editsave() {
        let that=this
        if(that.form.contract_signdate === null)
        {
          that.form.contract_signdate = ''
        }
        if(that.form.contract_return === null)
        {
          that.form.contract_return = ''
        }
        if(that.form.contract_start === null)
        {
          that.form.contract_start = ''
        }
        if(that.form.contract_end === null)
        {
          that.form.contract_end = ''
        }
      	that.$refs['form'].validate(async (valid) => {
      		if (valid) {
            that.fileUpload(async function(status) {
              if(that.isContractAdd)
              {
                that.form.customer_id = that.form.customer_name
              }
              ...
              +that.form.administrative+'$`'+that.form.operator_people+'$`'+that.form.contract_from+'$`'
              +that.form.project_simple+'$`'+that.form.work_content+'$`'+that.form.regard+'$`'+that.form.contract_filename+
              '$`'+that.form.contract_id+'$`'+that.form.customer_id+'$`'+that.form.parent_id+'$`'+that.form.contract_state+'$`'+that.form.warranty_bond+'}|^1|^SYS'
              console.log(sData)
              const res = await ExecSql(sData)
              if(res>0)
              {
      ...
      async fetchData() {
        this.selectloading=true
        let sData7 = '[Q]211{'+this.current_org+'$`1}|^SYS'
        const res7 = await QueryExec(sData7)
        this.customer_name_options=res7.data
        let sData = '[Q]676{合同性质}|^SYS'
        const res = await QueryExec(sData)
        this.contract_nature_options=res.data
