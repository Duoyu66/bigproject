<template>
  <div class="definition equipment_btn calc">
    <el-tabs
      ...
      class="box_center"
    >
      <el-tab-pane label="添加设备事故记录" class="center">
        <div class="usage">
          <AccidentForm ref="AccidentForm" @transferTable="transferTable"></AccidentForm>
          <div>
            <el-button
              class="purchase_btn"
              type="primary"
              @click="AccidentFormAdd()"
            >
              确 定
            </el-button>
          </div>
    ...
    <equipmentAccident ref="equipmentAccident" @fetchDataChild="fetchData"></equipmentAccident>
  </div>
</template>
...
import equipmentAccident from '../components/equipmentAccident.vue'
import AccidentForm from '../form/AccidentForm.vue'
  ...
  methods: {
    ...
    transferTable() {
      this.$baseConfirm('是否跳转到维护保养记录?如果要继续添加请点击取消。', null, async () => {
        this.$router.push('/device/equipmentAccident?tabs=1')
        this.fetchData()
      })
    },
    setSelectRows(val) {
    ...
    async fetchData() {
      ...
      let sData = '[Q]172{'+this.current_org+'$`%'+this.queryForm.manage_pid+'%$`%'+this.queryForm.device_name+'%$`'+((Number(this.queryForm.pageNo)-1)*Number(this.queryForm.pageSize)).toString()+'$`'+this.queryForm.pageSize+'}|^SYS'
