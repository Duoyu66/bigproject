<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <adjustItemForm ref="adjustItemForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></adjustItemForm> 
    </div> 
...
<script> 
  import adjustItemForm from '../form/adjustItemForm' 
  export default { 
    name: 'adjustItemDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['adjustItemForm']) { 
          this.$refs['adjustItemForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['adjustItemForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['adjustItemForm'].save(async (valid) => { 
          if (valid) { 
