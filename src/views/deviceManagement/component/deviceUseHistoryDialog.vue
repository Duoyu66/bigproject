<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <deviceUseHistoryForm ref="deviceUseHistoryForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></deviceUseHistoryForm> 
    </div> 
...
<script> 
  import deviceUseHistoryForm from '../form/deviceUseHistoryForm' 
  export default { 
    name: 'deviceUseHistoryDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['deviceUseHistoryForm']) { 
          this.$refs['deviceUseHistoryForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['deviceUseHistoryForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['deviceUseHistoryForm'].save(async (valid) => { 
          if (valid) { 
