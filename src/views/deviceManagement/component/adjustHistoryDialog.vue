<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center >
    <div style="max-height: calc(100vh - 250px); overflow-y: auto">
      <adjustHistoryForm ref="adjustHistoryForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></adjustHistoryForm>
    </div>
...
<script>
  import adjustHistoryForm from './../form/adjustHistoryForm.vue'
  export default {
    ...
    methods: {
      showEdit(row, formInfo, option) {
        ...
        this.dialogFormVisible = true
        if(this.$refs['adjustHistoryForm']) {
          this.$refs['adjustHistoryForm'].init(this.form);
        }
      },
      async close(event, status) {
        ...
        this.$refs['adjustHistoryForm'].close()
      ...
      save() {
        let that = this
        this.$refs['adjustHistoryForm'].save(async (valid) => {
          if (valid) {
