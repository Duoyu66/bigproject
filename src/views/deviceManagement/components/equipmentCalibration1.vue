      ...
      verify_confirm() {
        ...
        this.$refs['form'].validate(async (valid) => {
          that.fileUpload(async function(status) {
            ...
            if (valid) {
              let sData = '[Q]155{'+that.current_org+'$`'+that.form.device_id+'$`'+that.form.verify_date+'$`'+that.form.verify_org+'$`'
              +that.form.verify_result+'$`'+that.form.sender+'$`'+that.form.cert_id+'$`'+that.form.certificate_image+'$`'+that.form.cert_situation
              +'$`'+that.form.metering_no+'$`'+that.form.plan_id+'}|^2|^SYS'
              const res = await ExecSql(sData)
              console.log(sData)
              if(res>0)
              {
                that.$baseMessage('校检添加成功', 'success')
                that.dialogFormVisible=false
              }
              else
              {
                that.$baseMessage('校检添加失败', 'error')
              }
