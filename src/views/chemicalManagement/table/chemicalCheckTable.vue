  ...
  <!-- 化学品盘点记录表格 -->
	<div class="checkRTabBox">
		<el-table ref="tableSort" v-loading="listLoading" highlight-current-row :data="tablelist" border
			...
			@row-click="rowClick"  style="height: calc(95.5%);" class="checkRTabHigh">
		...
		<chemicalCheckDialog ref="chemicalCheckDialog" @reload="searchTable"></chemicalCheckDialog>
	</div>
</template>
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					...
					chemical_store: {
						edit: 'chemicalCheckEdit',
						delete: 'chemicalCheckAdd',
					}
				}
				switch (name) {
			...
			tableSortChange() {},
			handleEdit(row) {
				this.$refs['chemicalCheckDialog'].showEdit(row)
			},
			handleDelete(row) {
				let sqlcode = 542;
			...
			async fetchDataMain(pageInfo, treeNode, resolve) {
				...
				await this.search_fob(alist, this, 541, key_json)
			}
		},
	}
</script>
