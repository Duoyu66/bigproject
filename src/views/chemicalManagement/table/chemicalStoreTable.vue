<template>
	<div class="consumLeftBox">
		<el-table ref="tableSort" v-loading="listLoading" highlight-current-row :data="tablelist" border
			...
			@row-click="rowClick" style="height: calc(87%);" class="consumeleftHigh">
			...
			<el-table-column show-overflow-tooltip prop="model" label="规格型号" v-if="hideColFob('model')">
			</el-table-column>
			...
			<el-table-column show-overflow-tooltip label="存放地" prop="storage_place" v-if="hideColFob('storage_place')">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="是危险品" v-if="hideColFob('danger')">
		...
		<NullifyFormDialog ref="NullifyFormDialog" @reloadChildren="reloadChildren"></NullifyFormDialog>
		<chemicalCheckDialog ref="chemicalCheckDialog" @reloadChildren="reloadChildren"></chemicalCheckDialog>
		<chemicalConsumeDialog ref="chemicalConsumeDialog" @reloadChildren="reloadChildren"></chemicalConsumeDialog>
		<ChemicalBuyEdit ref="ChemicalBuyEdit" @reloadChildren="reloadChildren"></ChemicalBuyEdit>
		<RmMakeDialog ref="RmMakeDialog" @reloadChildren="reloadChildren"></RmMakeDialog>
		<chemicalStoreDialog ref="chemicalStoreDialog" @reload="searchTable"></chemicalStoreDialog>
	</div>
</template>
...
<script>
	...
	import chemicalCheckDialog from '../component/chemicalCheckDialog'
	import chemicalConsumeDialog from '../component/chemicalConsumeDialog'
	import chemicalStoreDialog from '../component/chemicalStoreDialog'
	import ChemicalBuyEdit from '../components/chemicalPurchase.vue'
	import RmMakeDialog from '../component/RmMakeDialog.vue'
	export default {
			...
			return {
				dropdown: {
					menu: [{
						...
						{
							command: 'addRmMake',
							permission_name: 'chemicalRmMakeAdd',
							label: '标准物质制作'
						},
						{
							command: 'Delete',
		...
		methods: {
			...
			handleBuy(row) {
				...
				this.$refs['ChemicalBuyEdit'].showEdit(undefined, info);
			},
			handleAddRmMake(row) {
				let info = {
					name: row.name,
				...
				this.$refs['RmMakeDialog'].showEdit(undefined, info);
			},
			handleEidtStore(row) {
				this.$refs['chemicalStoreDialog'].showEdit(row);
			},
			handleAddNullify(row) {
				this.$refs['NullifyFormDialog'].showEdit(undefined, row);
			},
			handleChemicalChecky(row) {
				let info = {
					...
					store_id: row.store_id,
					org_id: row.org_id,
					sys_quantity: row.quantity
				}
				this.$refs['chemicalCheckDialog'].showEdit(undefined, info);
			},
			handleConsumeAdd(row) {
				...
				this.$refs['chemicalConsumeDialog'].showEdit(undefined, info);
			},
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					chemical_consumption: {
						chemicalConsumeAdd: 'chemicalConsumptionadd',

					},
					chemical_check: {
						chemicalCheckAdd: 'chemicalInventoryAdd'
					},
					chemicalNullify: {
						nullify: 'voidRecordAdd'
					},
					chemical_store: {
						edit: 'chemicalStockEdit',
						delete: 'chemicalStockDelete',
						buy: 'chemicaBuyAdd',
						nullify: 'chemicaNulifyAdd',
						chemicalCheckAdd: 'chemicalCheckAdd',
						chemicalConsumeAdd: 'chemicalConsumeAdd'
					}
				}
				switch (name) {
				...
				// case 'chemicalNullify':
				let dropdowMenu = {
					chemical_consumption: ['chemicalConsumeAdd', 'empty'],
					chemical_check: ['chemicalCheckAdd', 'empty'],
					chemicalNullify: ['nullify', 'empty'],
					chemical_store: ['all'],
				}
			...
			handleDelete(row) {
				...
				let sqlcode = 557
				let tips = '确定删除？'
				this.$confirm('此操作将永久删除该化学品[' + row.name + ']的所有记录，包括购入、消耗、盘点等, 是否继续?', '提示', {
			...
			async fetchDataMain(pageInfo, treeNode, resolve) {
				...
				await this.search_fob(alist, this, 553, key_json)
			},
