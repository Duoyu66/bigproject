<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <personnelLedgerForm ref="personnelLedgerForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></personnelLedgerForm> 
    </div> 
...
<script> 
  import personnelLedgerForm from '../form/personnelLedgerForm' 
  export default { 
    name: 'personnelLedgerDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['personnelLedgerForm']) { 
          this.$refs['personnelLedgerForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['personnelLedgerForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['personnelLedgerForm'].save(async (valid) => { 
          if (valid) { 
