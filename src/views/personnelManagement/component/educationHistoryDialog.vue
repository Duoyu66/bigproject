<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <educationHistoryForm ref="educationHistoryForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></educationHistoryForm> 
    </div> 
...
<script> 
  import educationHistoryForm from '../form/educationHistoryForm' 
  export default { 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['educationHistoryForm']) { 
          this.$refs['educationHistoryForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['educationHistoryForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['educationHistoryForm'].save(async (valid) => { 
          if (valid) { 
