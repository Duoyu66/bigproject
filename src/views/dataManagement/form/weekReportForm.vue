	...
	<el-form ref="form" :model="form" :rules="rules">
		<el-row style="margin: 30px;">
			<el-col :span="span.full">
				<el-form-item label="周报日期" :label-width="formLabelWidth" prop="week_number">
					<el-date-picker
			...
			<el-col :span="span.full">
				<el-form-item label="周报标题" :label-width="formLabelWidth" prop="week_title">
					<el-input v-model="form.week_title" :placeholder="isReadOnly('请输入用途')" style="width:100%"
	...
	export default {
      ...
      getYearWeek(dateString){
          ...
          if(d<0){
			...
			async save() {
				let that = this
        let yy = new Date(that.form.week_number).getFullYear();
        let mm = new Date(that.form.week_number).getMonth()+1;
        let dd = new Date(that.form.week_number).getDate();
        let weeknum = this.getYearWeek(yy+'-'+mm+'-'+dd)
				let sData = '[Q]856{'+that.current_org_id+'$`'+that.person_id+'$`'+weeknum+'$`'+that.form.week_title+'$`'+that.form.week_content+'}|^2|^SYS'
