<template>
	<div class="draftPageHigh">
      ...
      <el-table-column show-overflow-tooltip prop="report_state" label="审核状态" width="100px" align="center" fixed="left">
      	<template slot-scope="scope">
      		<el-tag type="danger" v-if="scope.row.report_state=='0'">未审核
      		</el-tag>
      		<el-tag type="warning" v-else-if="scope.row.report_state=='1'">已审核
      		</el-tag>
      		<el-tag v-else>未知状态{{ scope.row.report_state }}
      		</el-tag>
      	</template>
      </el-table-column>
			<el-table-column show-overflow-tooltip label="项目名称" prop="org_name" width="150"></el-table-column>
      <el-table-column show-overflow-tooltip label="年月" prop="income_date" width="90"></el-table-column>
      <el-table-column show-overflow-tooltip label="检测项目" prop="object_name" min-width="120"></el-table-column>
			<el-table-column show-overflow-tooltip label="单价" prop="unitprice" min-width="120" ></el-table-column>
      <el-table-column show-overflow-tooltip label="数量" prop="count" min-width="120"></el-table-column>
      <el-table-column show-overflow-tooltip label="总金额" prop="amount" min-width="120" ></el-table-column>
      <el-table-column show-overflow-tooltip label="填报人" prop="person_name" min-width="120" ></el-table-column>
      <el-table-column show-overflow-tooltip label="审核人" prop="approve_name" min-width="120" ></el-table-column>
			<el-table-column show-overflow-tooltip label="操作" width="55px" align="center" fixed="right">
                ...
                v-show="ishasPermission('view',scope.row)">
								详情
							</el-dropdown-item>
...
<!-- 							<el-dropdown-item icon="el-icon-finished" divided
								...
								v-show="ishasPermission('export',scope.row)">
								导出
							</el-dropdown-item> -->
						</el-dropdown-menu>
					</el-dropdown>
    ...
    <monthReportSubDialog ref="monthReportSubDialog" :is-up-data.sync="main_dialog" :project_type="project_type">
    </monthReportSubDialog>
...
<script>
  ...
  import monthReportSubDialog from '../component/monthReportSubDialog'
	export default {
		name: 'sealApplyTable',
		components: {
      monthReportSubDialog
      ...
      project_type: {
        ...
        doc.generate("demo.docx");
			...
			handleAdd(row, parent_info, disable) {
				this.$refs['monthReportSubDialog'].showEdit(row, parent_info, disable)
			},
			handleAddLevel(row) {
				let json = {
        ...
        console.log(that)
          ...
          case 3:
			...
			//印章子表从表加载数据
			async fetchData1(pageInfo, treeNode, resolve) {
				let that = this
        if(!pageInfo) {
          pageInfo = {
        ...
        let sData = '[Q]885{'+this.current_org_id+'%$`'+that.project_type+'$`'+
        +((Number(pageInfo.pageNo)-1)*Number(this.queryForm.pageSize)).toString()+'$`'+this.queryForm.pageSize+'}|^SYS'
        const res = await QueryExec(sData);
        console.log(sData,res)
        res.data.forEach((item, index) => {
          if(item.salaryexpense==='') {
            item.salaryexpense='0'
          }
          if(item.travelexpensessum==='') {
            item.travelexpensessum='0'
          }
          if(item.consignexpense==='') {
            item.consignexpense='0'
          }
          if(item.oldexpense==='') {
            item.oldexpense='0'
          }
          if(item.expenseaccount==='') {
            item.expenseaccount='0'
          }
          item.sumexpense = Number(item.salaryexpense)+Number(item.travelexpensessum)+Number(item.consignexpense)+Number(item.oldexpense)+Number(item.expenseaccount)
        })
