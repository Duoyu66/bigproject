<template>
	<el-dialog :title="title" :visible.sync="dialogFormVisible" width="700px" @close="close" top="5vh">
		<div style="max-height: calc(100vh - 250px); overflow-y: auto">
			<weekReportForm ref="weekReportForm" :formData.sync="form" :type="0"></weekReportForm>
		</div>
...
<script>
	import weekReportForm from '../form/weekReportForm'
	export default {
		name: 'sealProcDialog',
		model: {
			prop: 'isUpData',
		...
		methods: {
			...
			close(status) {
				...
				this.$refs['weekReportForm'].close()
				this.dialogFormVisible = false
			},
			save() {
				let that = this
				this.$refs['weekReportForm'].save(async (valid) => {
					if (valid) {
						that.close(true)
					}
				})
