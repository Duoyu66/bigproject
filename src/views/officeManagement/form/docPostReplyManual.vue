<template>
	<div>
		<el-form ref="form" :model="form" :rules="rules">
			<el-row>
				<el-col :span="span.full">
					<el-form-item label="发文" :label-width="formLabelWidth" prop="receive_id">
						<el-select v-model="form.receive_id" style="width: 100%" filterable default-first-option
							:disabled="readonly" @change="receiveIdChange">
							<el-option v-for="data in selectData.receive_id" :key="data.receive_id"
								:label="data.doc_id+data.docTitle" :value="data.receive_id">
							</el-option>
						</el-select>
					</el-form-item>
				</el-col>
		...
		<div style="max-height: calc(100vh - 350px); overflow-y: auto">
			<dispatchRegisterForm ref="dispatchRegisterForm" :formData.sync="showForm" :type="1">
...
<script>
	...
	export default {
		...
		methods: {
			receiveIdChange(value) {
				this.ShowReply(value)
			},
			ShowReply(receive_id) {
				try {
					let that = this
					if (!receive_id) {
						receive_id = that.form.receive_id
					}
					let list = that.selectData.receive_id
					list = list.filter(function(o) {
						return o.receive_id == receive_id;
					});
					that.showForm = {}
					that.showForm = Object.assign(that.showForm, list[0])
				} catch (e) {
					console.log(e)
				}
