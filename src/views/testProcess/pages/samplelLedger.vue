<template>
	<div class="definition purchase equipment_btn calc">
		<sampleCollectPage ref="sampleCollectPageComplete" v-if="ifShow.all" stage="0">
		</sampleCollectPage>
	</div>
</template>

<script>
	import sampleCollectPage from '../component/sampleCollectPage'
	//sampleCollectPageHistoryTable
	export default {
		name: 'samplelLedger',
		components: {
			sampleCollectPage
		...
		methods: {
			...
			search_main_fob(data) {
				//search
				this.$refs['sampleCollectPageTable'].search(data)
			},
		},
	}
</script>
