<template>
  <!-- 委托申请 -->
	<div class="cardStyle">
        ...
        <el-tab-pane label="未委托" name="NoEntrust" v-show="permission.EntrustPageAdd">
          <EntrustPage ref="EntrustPageNoAcceptance" v-if="ifShow.NoEntrust" accepted="0">
          </EntrustPage>
        </el-tab-pane>
        <el-tab-pane label="已委托" name="NoAcceptance" v-show="permission.EntrustPageAdd">
        ...
        <el-tab-pane label="已受理" name="Acceptance" v-show="permission.Acceptance">
          <EntrustPage ref="EntrustPageAcceptance" v-if="ifShow.Acceptance" accepted="1">
          </EntrustPage>
        </el-tab-pane>
        <!-- <el-tab-pane label="未分配任务" name="reAllocation" v-show="permission.postExamine">
          <EntrustPage ref="EntrustPageRefuse" v-if="ifShow.reAllocation" accepted="-1">
          </EntrustPage>
        </el-tab-pane>

        <el-tab-pane label="试验" name="test" v-show="permission.postExamine">
          <EntrustPage ref="EntrustPagetest" v-if="ifShow.test" accepted="1" status="0">
          </EntrustPage>
        </el-tab-pane>
        <el-tab-pane label="复核" name="review">
          <EntrustPage ref="EntrustPageComplete" v-if="ifShow.review" accepted="1" status="1">
          </EntrustPage>
        </el-tab-pane>
        <el-tab-pane label="批准" name="approval">
          <EntrustPage ref="EntrustPageComplete" v-if="ifShow.approval" accepted="1" status="2">
          </EntrustPage>
        </el-tab-pane>
        <el-tab-pane label="计价中" name="valuation">
          <EntrustPage ref="EntrustPageComplete" v-if="ifShow.valuation" accepted="1" status="3">
          </EntrustPage>
        </el-tab-pane> -->
        <el-tab-pane label="所有委托" name="all">
...
.cardStyle .el-tabs__header:not(.el-footer .el-tabs__header ){
  ...
  background: #fff;border-radius: 5px;border: 1px solid #dbe0e6;
  padding: 10px;box-sizing: border-box;box-shadow: 0 1px 4px rgb(0 21 41 / 8%);
}
