<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <testDataForm ref="testDataForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></testDataForm> 
    </div> 
...
<script> 
  import testDataForm from '../form/testDataForm' 
  export default { 
    name: 'testDataDialog', 
    model: { 
      prop: 'isUpData', 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['testDataForm']) { 
          this.$refs['testDataForm'].init(this.form); 
        } 
      }, 
      ...
      save() { 
        let that = this 
        this.$refs['testDataForm'].save(async (valid) => { 
          if (valid) { 
