<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="1000px" @close="close">
    <el-row :gutter="5">
      ...
      <el-col :span="4">
        <el-table :data="data1" style="width: 100%;height: 640px;" @selection-change="setSelectRows"
          highlight-current-row row-key="id" border default-expand-all :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
      ...
      <el-col :span="20">
        ...
        <div id="paramCheck" style="margin:0px;padding:0px;height: 440px;"></div>
        <div style="margin:0px;padding:0px;height: 138px; padding-top: 20px;border-style: solid;border-width: 1px;border-color: #f0f0f0;">
          <el-form ref="form" :model="form" label-width="80px">
            <el-form-item label="复核意见" prop="author">
    ...
    <div slot="footer" class="dialog-footer" style="display: flex; justify-content: space-around;">
      <el-button type="primary" @click="" style="margin: 0;">复核通过</el-button>
      <el-button @click="close" style="margin: 0;">复核不通过</el-button>
      <el-button @click="close" style="margin: 0;">取 消</el-button>
...
<script>
  ...
  import {
    exportExcel
  } from '@/components/export/export.js'
  import {
    getparamlistone
  } from '@/api/table'
    ...
    methods: {
      async showEdit(row) {
        if (!row) {
          ...
          this.title = '试验复核-水泥试验'
          this.form = Object.assign({}, row)
        }
        const {
          data
        } = await getparamlistone(this.title)
        console.log(data)
        this.dialogFormVisible = true
        this.$nextTick(() => {
          $(function() {
            luckysheet.create({
              container: "paramCheck",
              showinfobar: false,
              showtoolbar: false,
              showsheetbar: false,
