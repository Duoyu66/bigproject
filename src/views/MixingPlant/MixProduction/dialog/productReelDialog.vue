<template>
	<el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center>
		<div style="max-height: calc(100vh - 250px); overflow-y: auto">
			<productReelForm ref="productReelForm" :formData.sync="form" :table_type="option.table_type" :type="type"
				:outOrgStatus="option.outOrgStatus"></productReelForm>
		</div>
...
<script>
	import productReelForm from '../form/productReelForm'
	export default {
		name: 'productReelDialog',
		model: {
			prop: 'isUpData',
			event: 'on-change'
		},
		...
		methods: {
			showEdit(row, formInfo, option) {
				...
				this.dialogFormVisible = true
				if (this.$refs['productReelForm']) {
					this.$refs['productReelForm'].init(this.form);
				}
			},
			async close(event, status) {
				...
				this.$refs['productReelForm'].close()
			...
			save() {
				let that = this
				this.$refs['productReelForm'].save(async (valid) => {
					if (valid) {
