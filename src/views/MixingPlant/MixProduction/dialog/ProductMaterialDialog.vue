<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <ProductMaterialForm ref="ProductMaterialForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></ProductMaterialForm> 
    </div> 
...
<script> 
  import ProductMaterialForm from '../form/ProductMaterialForm' 
  export default { 
    name: 'ProductMaterialDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['ProductMaterialForm']) { 
          this.$refs['ProductMaterialForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['ProductMaterialForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['ProductMaterialForm'].save(async (valid) => { 
          if (valid) { 
