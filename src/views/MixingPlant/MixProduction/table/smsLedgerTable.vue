<template>
  <div class="draftPageHigh">
    <el-table ref="branchInstitutions" v-loading="listLoading" :data="tablelist" border
      ...
      <el-table-column show-overflow-tooltip label="发送标题" prop="title" min-width="150px"></el-table-column>
      <el-table-column show-overflow-tooltip label="发送日期" prop="msg_date" min-width="100px"></el-table-column>
      <el-table-column show-overflow-tooltip label="发送内容" prop="message" min-width="200px"></el-table-column>
      <el-table-column show-overflow-tooltip label="用户名称" prop="user_name" min-width="100px"></el-table-column>
      <el-table-column show-overflow-tooltip label="移动电话" prop="mobile_phone" min-width="100px"></el-table-column>
      <el-table-column show-overflow-tooltip label="发送状态" prop="status" min-width="100px">
        <template #default="{ row }">
        	<el-tag v-if="row.status=='1'" type="success">已发送
        	</el-tag>
        	<el-tag v-else-if="row.status=='0'" type="info">未发送
        	</el-tag>
...
<script>
  ...
  import smsLedgerDialog from '../component/smsLedgerDialog'
  export default {
    name: 'smsLedgerTable',
    components: { smsLedgerDialog, },
    props: {
    ...
    methods: {
      ...
      async fetchDataMain(pageInfo, treeNode, resolve) {
        let that = this
        let select_list = [
          { field: 'risk_id',	type: 3,	level: 1	},
          { field: 'pageNo',	type: 3,	level: 2	},
          { field: 'pageSize',	type: 3,	level: 3	},
        ]
        that.queryForm.risk_id = that.primary_key
        await that.search_fob(select_list, that, 965, {
