<template>
  <div class="draftPageHigh">
    <el-table ref="branchInstitutions" v-loading="listLoading" :data="tablelist" border
      ...
      <el-table-column show-overflow-tooltip label="配方编号" prop="mix_ratio_id" min-width="150px"></el-table-column>
      <el-table-column show-overflow-tooltip label="强度等级" prop="strength" min-width="120px"></el-table-column>
      <!-- <el-table-column show-overflow-tooltip label="" prop="slump_set" min-width="120px"></el-table-column> -->
      <el-table-column show-overflow-tooltip label="合同编号" prop="contract_id" min-width="120px"></el-table-column>
      <el-table-column show-overflow-tooltip label="水胶比" prop="w_g_ratio" min-width="120px"></el-table-column>
      <el-table-column show-overflow-tooltip label="砂" prop="sand_ratio" min-width="120px"></el-table-column>
      <el-table-column show-overflow-tooltip label="养护条件" prop="curing_mode" min-width="120px"></el-table-column>
      <el-table-column show-overflow-tooltip label="调整来自" prop="adjust_from" min-width="100px"></el-table-column>
      <el-table-column show-overflow-tooltip label="调整原因" prop="adjust_reason" min-width="120px"></el-table-column>
...
<script>
  ...
  import ratioLedgerDialog from '../component/ratioLedgerDialog'
  export default {
    name: 'ratioLedgerTable',
    components: { ratioLedgerDialog, },
    props: {
    ...
    watch: {
      ...
      rowClickData(newVal, oldVal) {
        console.log('rowClickData', newVal)
        if (newVal && newVal.proport_id) {
          this.$emit('update:primary_key', Number(newVal.proport_id));
        }
      },
    },
    data() {
    ...
    methods: {
      ...
      // clearChildren() {
      //   this.rowClickData = { proport_id:-1, }
      // },
      // fencthRelod(row) {
      //   if (!row) {
      ...
      async fetchDataMain(pageInfo, treeNode, resolve) {
        ...
        let select_list = [
          ...
          { field: 'strength',	type: 3,	level: 3	},
          { field: 'pageNo',	type: 3,	level: 4	},
          { field: 'pageSize',	type: 3,	level: 5	},
        ]
        that.queryForm.org_id=that.current_and_chrilren_org
        console.log(that.queryForm)
        await that.search_fob(select_list, that, 998, {
      ...
      async fetchData1(pageInfo, treeNode, resolve) {
        ...
        console.log(that.queryForm)
        await that.search_fob(select_list, that, 1023, {
          loading: 'listLoading', //加载画面
          data: 'tablelist', //数据存放位置
          total: 'total', //数据数量，根据该数量分页
          queryForm: 'queryForm', //查询数据表单，请将分页条件也放置在其中
