<template>
	<div class="draftPageHigh">
		<el-table ref="branchInstitutions" v-loading="listLoading" :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip label="产生时间" prop="check_date" width="150px">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="问题级别" prop="risk_level" width="80px">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="问题来源" prop="risk_source" width="100px" v-if="false">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="扣分值" prop="credit" width="120px">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="责任人" prop="charge_person" min-width="150px">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="状态" prop="r_status" width="120px">
				<template #default="{ row }">
					<el-tag v-if="row.r_status=='0'" type="danger">待整改
					</el-tag>
					<el-tag v-else-if="row.r_status=='1'" type="warning">待整改确认
					</el-tag>
					<el-tag v-else-if="row.r_status=='2'" type="info">待销号
					</el-tag>
					<el-tag v-else-if="row.r_status=='3'" type="success">已完成
					</el-tag>
					</el-tag>
					<el-tag v-else>{{row.r_status}}
					</el-tag>
				</template>
			</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			fetchData(pageInfo, treeNode, resolve) {
				...
				switch (that.type) {
					...
					case 12:
						...
						break
					case 20:
						that.fetchDataMixMap(pageInfo, treeNode, resolve);
						break;
				}
			},
			...
			async fetchDataMixMap(pageInfo, treeNode, resolve) {
				let that = this
				let select_list = [{
						field: 'pageNo',
						type: 3,
				...
				await that.search_fob(select_list, that, 1027, {
					loading: 'listLoading', //加载画面
					data: 'tablelist', //数据存放位置
					total: 'total', //数据数量，根据该数量分页
					queryForm: 'queryForm', //查询数据表单，请将分页条件也放置在其中
