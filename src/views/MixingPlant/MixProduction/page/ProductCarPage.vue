	...
	<div class="table-container purchase equipment_btn">
		<div class="baseBox" style="margin-bottom: 10px;">
				...
				<vab-query-form-right-panel style="width: 100%">
					<el-form ref="form" :model="queryForm" :inline="true" @submit.native.prevent>
						<el-form-item style="padding-right: 10px;">
							<el-date-picker v-model="queryForm.start_start_time" align="right" type="datetime"
								placeholder="选择运输开始时间" :picker-options="pickerOptions" style="width: 100%"
								:format='dateFormat' :value-format="dateFormat">
							</el-date-picker>
						</el-form-item>
						<el-form-item style="padding-right: 10px;">
							<el-date-picker v-model="queryForm.end_start_time" align="right" type="datetime"
								placeholder="选择运输结束时间" :picker-options="pickerOptions" style="width: 100%"
								:format='dateFormat' :value-format="dateFormat">
							</el-date-picker>
						</el-form-item>
						<el-form-item style="padding-right: 10px;">
			...
			<el-container>
				<el-main style="height: calc(37vh); min-height: 275px;" class="box1">
					<!-- ProductCarTable -->
					<ProductCarTable ref="ProductCarTable" style="height: calc(100%);min-height: 300px" :type="1"
						@fatherMethod="fatherMethod" :selectForm.sync="queryForm" :primary_key.sync="primary_key"
						:permission="permission" :isOnlyShowQualified="isOnlyShowQualified"
						:isOnlyShowError="isOnlyShowError">
					</ProductCarTable>
		...
		<el-footer class="table_auto">
			<el-tabs type="border-card" style="height: calc(42.5164vh)" @tab-click="handleClick" v-model="activeName">
				<el-tab-pane label="盘次记录" name="productReelTable">
					<productReelTable ref="productReelTable" style="height: calc(34vh)" v-if="ifShow.productReelTable"
						:primary_key.sync="primary_key" :permission="permission" :type="10">
					</productReelTable>
				</el-tab-pane>
...
<script>
	...
	import ProductCarTable from '../table/ProductCarTable'
	import riskTable from '../table/riskTable'
	import productReelTable from '../table/productReelTable'
	import RatioDetailTable from '../table/ratioDetailTable'
	...
	export default {
		...
		data() {
			return {
				...
				queryForm: {
					...
					param: '',
					class_name: '',
					start_start_time: getdayEx('day', -7, dateFormat(1))
				},
				primary_key: 0,
				permission: {},
				ifShow: {
		...
		methods: {
			...
			search_main_fob() {
				this.$refs['ProductCarTable'].search(this.queryForm)
			},
			reload_min(row) {
				this.$refs['ProductCarTable'].fetchData(row)
			},
			handleAddd() {
			...
			fatherMethod(row) {
				console.log('ceshi');
				this.$refs['truckTravel'].init(row)
			}
		},
	}
</script>
