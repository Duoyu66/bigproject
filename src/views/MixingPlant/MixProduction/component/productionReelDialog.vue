<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <productionReelForm ref="productionReelForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></productionReelForm> 
    </div> 
...
<script> 
  import productionReelForm from '../form/productionReelForm' 
  export default { 
    name: 'productionReelDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['productionReelForm']) { 
          this.$refs['productionReelForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['productionReelForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['productionReelForm'].save(async (valid) => { 
          if (valid) { 
