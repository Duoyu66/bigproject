<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <riskLedgerForm ref="riskLedgerForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></riskLedgerForm> 
    </div> 
...
<script> 
  import riskLedgerForm from '../form/riskLedgerForm' 
  export default { 
    name: 'riskLedgerDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['riskLedgerForm']) { 
          this.$refs['riskLedgerForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['riskLedgerForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['riskLedgerForm'].save(async (valid) => { 
          if (valid) { 
