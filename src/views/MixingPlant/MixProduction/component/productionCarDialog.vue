<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center >
    <div style="max-height: calc(100vh - 250px); overflow-y: auto">
      <productionCarForm ref="productionCarForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></productionCarForm>
    </div>
...
<script>
  import productionCarForm from '../form/productionCarForm' 
  export default {
    name: 'productionCarDialog',
    model: {
      prop: 'isUpData',
      event: 'on-change'
    },
    ...
    methods: {
      showEdit(row, formInfo, option) {
        ...
        this.dialogFormVisible = true
        if(this.$refs['productionCarForm']) {
          this.$refs['productionCarForm'].init(this.form);
        }
      },
      async close(event, status) {
        ...
        this.$refs['productionCarForm'].close()
      ...
      save() {
        let that = this
        this.$refs['productionCarForm'].save(async (valid) => {
          if (valid) {
