<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <ratioLedgerForm ref="ratioLedgerForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></ratioLedgerForm> 
    </div> 
...
<script> 
  import ratioLedgerForm from '../form/ratioLedgerForm' 
  export default { 
    name: 'ratioLedgerDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['ratioLedgerForm']) { 
          this.$refs['ratioLedgerForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['ratioLedgerForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['ratioLedgerForm'].save(async (valid) => { 
          if (valid) { 
