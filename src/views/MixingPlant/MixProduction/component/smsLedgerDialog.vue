<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <smsLedgerForm ref="smsLedgerForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></smsLedgerForm> 
    </div> 
...
<script> 
  import smsLedgerForm from '../form/smsLedgerForm' 
  export default { 
    name: 'smsLedgerDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['smsLedgerForm']) { 
          this.$refs['smsLedgerForm'].init(this.form); 
        } 
      }, 
      ...
      save() { 
        let that = this 
        this.$refs['smsLedgerForm'].save(async (valid) => { 
          if (valid) { 
