<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <riskSlaveLedgerForm ref="riskSlaveLedgerForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></riskSlaveLedgerForm> 
    </div> 
...
<script> 
  import riskSlaveLedgerForm from '../form/riskSlaveLedgerForm' 
  export default { 
    name: 'riskSlaveLedgerDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['riskSlaveLedgerForm']) { 
          this.$refs['riskSlaveLedgerForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['riskSlaveLedgerForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['riskSlaveLedgerForm'].save(async (valid) => { 
          if (valid) { 
