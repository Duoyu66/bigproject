<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <taskLedgerForm ref="taskLedgerForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></taskLedgerForm> 
    </div> 
...
<script> 
  import taskLedgerForm from '../form/taskLedgerForm' 
  export default { 
    name: 'taskLedgerDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['taskLedgerForm']) { 
          this.$refs['taskLedgerForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['taskLedgerForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['taskLedgerForm'].save(async (valid) => { 
          if (valid) { 
