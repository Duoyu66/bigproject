<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <riskFlowForm ref="riskFlowForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></riskFlowForm> 
    </div> 
...
<script> 
  import riskFlowForm from '../form/riskFlowForm' 
  export default { 
    name: 'riskFlowDialog', 
    model: { 
      prop: 'isUpData', 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['riskFlowForm']) { 
          this.$refs['riskFlowForm'].init(this.form); 
        } 
      }, 
      ...
      save() { 
        let that = this 
        this.$refs['riskFlowForm'].save(async (valid) => { 
          if (valid) { 
