<template> 
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px" @close="close" top="5vh" center > 
    <div style="max-height: calc(100vh - 250px); overflow-y: auto"> 
      <ratioDetailForm ref="ratioDetailForm" :formData.sync="form" :table_type="option.table_type" :type="type" :outOrgStatus="option.outOrgStatus"></ratioDetailForm> 
    </div> 
...
<script> 
  import ratioDetailForm from '../form/ratioDetailForm' 
  export default { 
    name: 'ratioDetailDialog', 
    model: { 
      prop: 'isUpData', 
      event: 'on-change' 
    }, 
    ...
    methods: { 
      showEdit(row, formInfo, option) { 
        ...
        this.dialogFormVisible = true 
        if(this.$refs['ratioDetailForm']) { 
          this.$refs['ratioDetailForm'].init(this.form); 
        } 
      }, 
      async close(event, status) { 
        ...
        this.$refs['ratioDetailForm'].close() 
      ...
      save() { 
        let that = this 
        this.$refs['ratioDetailForm'].save(async (valid) => { 
          if (valid) { 
