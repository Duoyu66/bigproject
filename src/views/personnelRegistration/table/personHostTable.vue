<template>
	<div>
		<el-table ref="personel" v-loading="listLoading" :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip label="设计时间" prop="design_date" align="center" width="120px">
			</el-table-column>
			<el-table-column show-overflow-tooltip prop="prj_name" label="项目名称" min-width="200px">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="规模" prop="scale" width="150px" align="center">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="是否主持人" prop="isHost" align="center" width="100px">
			</el-table-column>
			<el-table-column show-overflow-tooltip prop="witness" label="证明人" width="120px">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="证明人电话" prop="witness_phone" align="center" width="120px">
			</el-table-column>
			...
			<el-table-column show-overflow-tooltip prop="approve" label="审核状态" align="center" width="150px">
				<template slot-scope="scope">
					...
					<span style="margin-left: 10px"
						v-else-if="scope.row.approve ==='-3'">作废[{{scope.row.reason}}]</span>
				</template>
			</el-table-column>
			...
			<el-table-column show-overflow-tooltip label="操作" width="170px" align="center" fixed="right">
				<template #default="{ row }">
					<el-button class="normal" @click="ExamineHost(row)" v-show="ishasPermission('examine',row)">
						审核
					</el-button>
					<el-button class="normal" @click="EditPersonelHost(row)" v-show="ishasPermission('edit',row)">
						修改
					</el-button>
		...
		<personnelHost ref="personnelHost" :isUpData.sync="main_dialog"></personnelHost>
		<HostExamine ref="HostExamine" :isUpData.sync="main_dialog"></HostExamine>
	</div>
</template>
...
<script>
	...
	import HostExamine from '../components/HostExamine'
	export default {
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				switch (name) {
					case 'add':
						...
						break
					case 'edit':
						permission_name = 'host_edit'
						break
					case 'delete':
						permission_name = 'host_delete'
						break
					case 'examine':
						permission_name = 'host_examine'
						break
					default:
			...
			ExamineHost(row) {
				this.$refs['HostExamine'].showEdit(row)
			},
			EditPersonelHost(row) {
				this.$refs['personnelHost'].showEdit(row)
			},
			handleDeletemodify(row) {
				let that = this
				that.delete_from_fob(that, row, 175, {
			...
			async fetchData1() {
				...
				that.search_fob(select_list, that, 88, {
