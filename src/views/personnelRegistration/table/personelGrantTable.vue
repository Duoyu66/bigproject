<template>
	<div>
		<el-table ref="personel" v-loading="listLoading" :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip label="授权名称" prop="empower_name" min-width="200px"></el-table-column>
			<el-table-column show-overflow-tooltip label="授权日期" prop="empower_date" align="center">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="有效日期" prop="validity_date" align="center">
			</el-table-column>
				...
				<template #default="{ row }">
					<el-dropdown trigger="click" placement="left-start">
						...
						<el-dropdown-menu slot="dropdown" class="more" style="min-width: 115px">
							...
							<el-button @click="EditPersonelGrant(row)" v-show="ishasPermission('edit',row)">
								修改
							</el-button>
		...
		<personnelGrant ref="personnelGrant" :isUpData.sync="main_dialog"></personnelGrant>
		<grantExamine ref="grantExamine" :isUpData.sync="main_dialog"></grantExamine>
	</div>
</template>
...
<script>
	...
	import grantExamine from '../components/grantExamine'
	export default {
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				switch (name) {
					case 'add':
						...
						break
					case 'edit':
						permission_name = 'empower_edit'
						break
					case 'delete':
						permission_name = 'empower_delete'
						break
					case 'examine':
						permission_name = 'empower_examine'
						break
					default:
			...
			examine(row) {
				this.$refs['grantExamine'].showEdit(row)
			},
			EditPersonelGrant(row) {
				this.$refs['personnelGrant'].showEdit(row)
			},
			handleDeletemodify(row) {
				let that = this
				that.delete_from_fob(that, row, 138, {
					//"selectRows": "selectRows", //复选框选中数据
					"fetchData": "fetchData", //加载数据
					"main_key": "empower_id", //主键
					"file": {
						empower_img: 12
					},
				});
			...
			async fetchData1() {
				...
				that.search_fob(select_list, that, 134, {
