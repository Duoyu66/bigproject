<template>
	<div>
		<el-table ref="personel" v-loading="listLoading" :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip label="成果名称" prop="awards_name" min-width="150px">
			</el-table-column>
			<el-table-column show-overflow-tooltip prop="finish_date" label="完成日期" align="center" width="130px">
			</el-table-column>
			<el-table-column show-overflow-tooltip prop="reward_date" label="获奖日期" align="center" width="130px">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="奖励单位" prop="revard_unit" min-width="150px">
			</el-table-column>
			<el-table-column show-overflow-tooltip label="等级" prop="awards_class" align="center" width="80px">
			</el-table-column>
			<el-table-column show-overflow-tooltip prop="partake" label="参与情况" align="center" width="150px">
				<template slot-scope="scope">
					<span style="margin-left: 10px" v-if="scope.row.partake =='1'">是</span>
					<span style="margin-left: 10px" v-if="scope.row.partake =='2'">否</span>
				</template>
			</el-table-column>
			<el-table-column show-overflow-tooltip prop="remarks" label="备注" align="center" width="110px">
			</el-table-column>
			...
			<el-table-column show-overflow-tooltip label="操作" width="170px" align="center" fixed="right">
				<template #default="{ row }">
					<el-button class="normal" @click="examine_fob(row)" v-show="ishasPermission('examine',row)">
						审核
					</el-button>
					<el-button class="normal" @click="EditPersonelTech(row)" v-show="ishasPermission('edit',row)">
						修改
					</el-button>
		...
		<techExamine ref="techExamine" :isUpData.sync="main_dialog"></techExamine>
		<personnelTech ref="personnelTech" :isUpData.sync="main_dialog"></personnelTech>
	</div>
</template>
...
<script>
	...
	import techExamine from '../components/techExamine'
	...
	export default {
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				switch (name) {
					case 'add':
						...
						break
					case 'edit':
						permission_name = 'award_edit'
						break
					case 'delete':
						permission_name = 'award_delete'
						break
					case 'examine':
						permission_name = 'award_examine'
						break
					default:
			...
			examine_fob(row) {
				this.$refs['techExamine'].showEdit(row)
			},
			EditPersonelTech(row) {
				this.$refs['personnelTech'].showEdit(row)
			},
			handleDeletemodify(row) {
				let that = this
				that.delete_from_fob(that, row, 135, {
			...
			async fetchData1() {
				...
				that.search_fob(select_list, that, 110, {
