	...
	<span>
		<el-form ref="purchase_from" :model="form" :rules="rules" style="padding: 0px 0px;">
			<el-row>
				...
				<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
					<el-form-item prop="calibrate_date" label="计划实施时间" :label-width="formLabelWidth">
				...
				<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
					<el-form-item prop="calibrate_org" label="校验机构" :label-width="formLabelWidth">
						<el-input v-model="form.calibrate_org" :placeholder="isReadOnly('校验机构...')" type="text"
							style="width: 100%" :readonly="readonly" :maxlength='25'>
						</el-input>
					</el-form-item>
				</el-col>
				...
				<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
					<el-form-item prop="" label="" label-width="50px">
						<el-table ref="check_show" v-loading="table.check_show.listLoading" highlight-current-row
							...
							<el-table-column show-overflow-tooltip label="校检方式">
								<template #default="{ row }">
									<el-input v-model="row.check_method" :placeholder="isReadOnly('校检方式...')"
		...
		<el-dialog title="选择标准物质" :visible.sync="dialog.dialogFormVisible" top="3vh" width="80%" append-to-body
			...
			<div style="max-height: calc(100vh - 220px); overflow-y: auto">
				<verificationPlanSelectTable ref="SelectTable" v-model="form.select_store" @UpdateSelect='UpdateSelect'>
				</verificationPlanSelectTable>
			</div>
...
<script>
	...
	import verificationPlanSelectTable from '../table/verificationPlanSelectTable'
	export default {
		name: 'verificationPlanForm',
		components: {
			verificationPlanSelectTable,
		},
		props: {
		...
		methods: {
			...
			handleDelete(row) {
				let index = this.table.check_show.tablelist.indexOf(row)
				if (index < 0) {
					console.log('index<0')
					return
				}
				...
				try {
					...
					this.table.check_show.tablelist.splice(index, 1);
				} catch (e) {
					console.log(e)
				}
				this.checkShowQuery()
