<template>
	<el-dialog :title="title" :visible.sync="dialogFormVisible" top="3vh" destroy-on-close>
		<div style="max-height: calc(100vh - 220px); overflow-y: auto">
			<substanceForm ref="substanceForm" :formData="form"></substanceForm>
		</div>
...
<script>
	import substanceForm from '../form/substanceForm.vue'

	export default {
		name: 'substanceDialog',
		components: {
			substanceForm,
		},
		data() {
		...
		methods: {
			...
			handleUpdate() {
				let that = this
				this.$refs['substanceForm'].save(function(status) {
					if (status) {
