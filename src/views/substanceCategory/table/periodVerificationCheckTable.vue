<template>
	<div>
		<el-table ref="tableSort" v-loading="listLoading" highlight-current-row :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip label="核查状态" width="78px">
				<template #default="{ row }">
					<el-tag type="success" v-if="row.history_id">已核查</el-tag>
					<el-tag type="warning" v-else>未核查</el-tag>
				</template>
			</el-table-column>
...
<script>
	...
	import TableEdit from '../component/materiaExamineAndVerifyCheckDialog'
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					periodVerificationCheck: {
						editCheck: 'PVCheckEdit',
						delete: 'PVCheckDelete',
			...
			handleDelete(row) {
				const vue = this
				let tips = '此操作将永久删除' + row.store_name + '的核查计划和向关的核查记录' + ', 是否继续?'
				let sqlcode = 594
			...
			async fetchData(pageInfo, treeNode, resolve) {
				...
				switch (that.type) {
					case 'periodVerificationCheck':
						await that.fetchDataperiodVerificationCheck(pageInfo, treeNode, resolve)
			...
			async fetchDataperiodVerificationCheck(pageInfo, treeNode, resolve) {
				let alist = [{
						field: 'org_id',
						type: 3,
						level: 1
				...
				await this.search_fob(alist, this, 588, key_json)
			},
