	...
	<div style="height: calc(60vh);">
		<div class="voidrecord" style="margin-bottom: 16px">
			<el-form ref="form" :model="queryForm" :inline="true" @submit.native.prevent
				...
				<el-form-item>
					<el-switch v-model="queryForm.isShowSelect" active-text="仅显示需要核查的标准物质" inactive-text=""
		...
		<el-table ref="tableSort" v-loading="listLoading" highlight-current-row :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip prop="implementation_date" label="上次核查日期" width="130px">
			</el-table-column>
			<el-table-column show-overflow-tooltip prop="nextimplementation_date" label="下次核查日期" width="130px">
			</el-table-column>
			<el-table-column show-overflow-tooltip prop="check_method" label="上次核查方式">
			</el-table-column>
...
<script>
	...
	export default {
		name: 'materiaExamineAndVerifySelectTable',
		components: {
		...
		methods: {
			...
			async fetchData(pageInfo, treeNode, resolve) {
				...
				switch (that.type) {
					case 'check_Period':
						await that.fechDataPeriod(pageInfo, treeNode, resolve)
						this.toggleSelection()
						break
				}
			},
			async fechDataPeriod() {
				let alist = [{
						field: 'mat_name',
						type: 2,
						level: 1,
				...
				await this.search_fob(alist, this, 584, key_json)
			},
		},
	}
</script>
