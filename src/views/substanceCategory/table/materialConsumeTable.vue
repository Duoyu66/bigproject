<template>
	<div>
		<el-table ref="tableSort" v-loading="listLoading" highlight-current-row :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip label="使用人" prop="user"></el-table-column>
...
<script>
	...
	import TableEdit from '../component/materialConsumeDialog'
	export default {
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					materialConsume: {
						add: 'consumptionRecordAdd',
						edit: 'consumptionRecordEdit',
						delete: 'consumptionRecordDelete',
					},
					chemical_store: {
						add: 'materialConsumeAdd',
						edit: 'materialConsumeEdit',
						delete: 'materialConsumeDelete',
					},
				}
				switch (name) {
			...
			handleDelete(row) {
				const vue = this
				let tips = '此操作将永久删除' + row.user + '在' + row.use_date + '使用' + row.rm_store_name + '类型[' +
					row.mat_name +
					']的记录' + ', 是否继续?'
				let sqlcode = 583
			...
			async fetchData(pageInfo, treeNode, resolve) {
				...
				switch (that.type) {
					case 'materialConsume':
						await that.fetchDataMaterialConsume(pageInfo, treeNode, resolve)
			...
			async fetchDataMaterialConsume(pageInfo, treeNode, resolve) {
				let alist = [{
						field: 'org_id',
						type: 3,
						level: 1
					...
					{
						field: 'record_date_start',
						type: 3,
						level: 9
					},
					{
						field: 'record_date_end',
						type: 3,
						level: 10
					},
					{
				...
				await this.search_fob(alist, this, 580, key_json)
			},
			async fetchDataStore(pageInfo, treeNode, resolve) {
				...
				await this.search_fob(alist, this, 617, key_json)
			}
		},
	}
</script>
