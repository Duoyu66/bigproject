<template>
	<div>
		<el-table ref="tableSort" v-loading="listLoading" highlight-current-row :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip prop="check_method" label="校检方式" width="100px"></el-table-column>
			<el-table-column show-overflow-tooltip label="校检状态" width="78px">
				<template #default="{ row }">
					<el-tag type="success" v-if="row.history_id">已校检</el-tag>
					<el-tag type="warning" v-else>未校检</el-tag>
				</template>
			</el-table-column>
...
<script>
	...
	import TableEdit from '../component/verificationPlanCheckDialog'
	import HistoryDialog from '../component/verificationPlanCheckHistoryDialog'
	import HistoryShowDialog from '../component/verificationPlanCheckHistoryShowDialog'
	export default {
		name: 'verificationPlanCheckTable',
		components: {
			TableEdit,
			HistoryDialog,
			HistoryShowDialog
		...
		methods: {
			...
			async SetoffsetHeight() {
				let $el = await this.Get$el();
				let offsetHeight = $el.offsetHeight
				console.log('SetoffsetHeight', offsetHeight)
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					verificationPlanCheck: {
						editCheck: 'verificationCheckEdit',
						delete: 'verificationCheckDelete',
						addhistory: 'verificationHistoryAdd',
						edithistory: 'verificationHistoryEdit',
					},
			...
			handleDelete(row) {
				const vue = this
				let tips = '此操作将永久删除' + row.store_name + '的校检计划和向关的校检记录' + ', 是否继续?'
				let sqlcode = 610
				this.delete_from_fobEx(vue, row, sqlcode, {
					//"selectRows": "selectRows", //复选框选中数据
					"fetchData": "fetchData", //加载数据
					"main_key": "check_id", //主键
					'confirmMsg': tips,
					'successMsg': '删除成功',
					'fail': '删除失败,请刷新查看是否删除',
				});
			...
			async fetchData(pageInfo, treeNode, resolve) {
				...
				switch (that.type) {
					case 'verificationPlanCheck':
						await that.fetchDataVerificationPlanCheck(pageInfo, treeNode, resolve)
			...
			async fetchDataVerificationPlanCheck(pageInfo, treeNode, resolve) {
				let alist = [{
						field: 'org_id',
						type: 3,
						level: 1
				...
				await this.search_fob(alist, this, 603, key_json)
			},
