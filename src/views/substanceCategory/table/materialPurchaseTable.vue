<template>
	<div>
		<el-table ref="tableSort" v-loading="listLoading" highlight-current-row :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip prop="buy_date" label="采购日期"></el-table-column>
			...
			<el-table-column show-overflow-tooltip prop="buyer" label="购买人"></el-table-column>
			<el-table-column show-overflow-tooltip prop="element" label="主要成分"></el-table-column>
...
<script>
	...
	import TableEdit from '../component/materialPurchaseDialog'
	export default {
		name: 'materialNullifyTable',
		components: {
			TableEdit
		},
		props: {
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					...
					chemical_store: {
						add: 'materialBuyAdd',
						edit: 'materialBuyEdit',
						delete: 'materialBuyDelete',
					},
				}
				switch (name) {
			...
			handleDelete(row) {
				const vue = this
				let tips = '此操作将永久删除' + row.buyer + '在' + row.buy_date + '购入的' + row.rm_store_name + '类型[' + row.mat_name +
					']' + ', 是否继续?'
				let sqlcode = 576
			...
			async fetchData(pageInfo, treeNode, resolve) {
				...
				switch (that.type) {
					case 'materialPurchase':
						await that.fetchDataMaterialPurchase(pageInfo, treeNode, resolve)
			...
			async fetchDataMaterialPurchase(pageInfo, treeNode, resolve) {
				let alist = [{
						field: 'org_id',
						type: 3,
						level: 1
				...
				await this.search_fob(alist, this, 575, key_json)
			},
			async fetchDataStore(pageInfo, treeNode, resolve) {
				...
				await this.search_fob(alist, this, 616, key_json)
			}
		},
	}
</script>
