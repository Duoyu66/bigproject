<template>
	<div>
		<el-table ref="tableSort" v-loading="listLoading" highlight-current-row :data="tablelist" border
			...
			<el-table-column show-overflow-tooltip prop="calibrate_date" label="核查日期" width="140px">
			</el-table-column>
...
<script>
	...
	export default {
		name: 'verificationPlanCheckHistoryTable',
		components: {
			HistoryDialog,
			HistoryShowDialog
		},
		...
		methods: {
			...
			ishasPermission(name, row) {
				...
				let permissionRegister = {
					verificationPlanHistory: {
						edithistory: 'verificationHistoryEdit',
						delete: 'verificationHistoryDelete',
						addhistory: 'verificationHistoryAdd',
					},
					chemical_store: {
						edithistory: 'RMPeriodCheckEdit',
						delete: 'RMPeriodCheckDelete',
					},
			...
			handleDelete(row) {
				const vue = this
				let tips = '此操作将永久删除' + row.handler + '在' + row.calibrate_date + '执行的核查记录' + ', 是否继续?'
				let sqlcode = 611
			...
			async fetchData(pageInfo, treeNode, resolve) {
				...
				switch (that.type) {
					case 'verificationPlanHistory':
						await that.fetchDataVerificationPlanHistory(pageInfo, treeNode, resolve)
						break
					case 'chemical_store':
			...
			async fetchDataVerificationPlanHistory(pageInfo, treeNode, resolve) {
				let alist = [{
						field: 'org_id',
						type: 3,
						level: 1
					...
					{
						field: 'calibrate_date_start',
						type: 3,
						level: 8
					},
					{
						field: 'calibrate_date_end',
						type: 3,
						level: 9
					},
					{
				...
				await this.search_fob(alist, this, 606, key_json)
			},
			async fetchDataStore(pageInfo, treeNode, resolve) {
				...
				await this.search_fob(alist, this, 619, key_json)
			},
		},
	}
</script>
